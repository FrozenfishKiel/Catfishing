...
| ------- | ----------------------------------------------------------------------- | -------------------------------------------- | ------------------------------------ | ----------------------------- |
| COLD-01 | 记录当前 commit/工作树状态，确认不把 `Saved/Verification/.../runtime_safe_null*` 当主工程；同时确认源码目录已是 `AbilitySystem/`、`Character/`、`Condition/`、`Framework/Game/`、`Online/`、`UI/` 等领域目录 | 主项目仍使用 `Config/DefaultEngine.ini` 的 Steam 配置，且不是验证副本 | 主工程未被测试副本覆盖；源码目录不是旧的模块根平铺结构 | 截图工作区路径、当前 commit、配置和目录树；停止后续操作 |
