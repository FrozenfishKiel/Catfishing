...
`LineLoad` 仍表达鱼游向在线方向上的投影，并参与磨损和断线负载。鱼与猫先各自产生候选端点；`Locked/Reeling/FreeSpool` 分别保持、缩短或允许被动增加 `L_paid`。全部变化只汇入一个约束误差，再按双方质量分配鱼端位置修正与猫端单步速度冲量。猫端不再使用容易被行走制动抵消的持续 `AddForce`。
