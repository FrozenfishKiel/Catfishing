...
| 图 05 | 主界面设置：游戏 | 深色透明设置页；标签为游戏、画面、声音、控制；游戏页包含语言、语音输入模式、麦克风设备、震动；2026-09-17 更新后三种语音模式合并旧开关，麦克风支持设备选择；无实际服务时显示不可用原因 | 可用项左右切换选项值；`R` 恢复默认；`Enter` 应用；`Esc` 返回；禁用项不提交或保存偏好 |
...
2026-09-17 最新人工决定：接入游戏分类的“麦克风设备”选择，以及禁用、常开、按住 V 说话三个输入模式，替代此前暂不可用的当前口径。主菜单和局内菜单读取同一设备 Model；选择、取消、恢复默认仅改草稿，应用后保存成功选择。当前实现限定 UE 5.8 Win64 Steam；实际双机收音验收与引擎设备限制见《主界面子技术文档》末尾，不把编译通过当作实机通话成功。
...
- 三种语音输入模式与麦克风选择均按 2026-09-17 用户确认接入；服务未就绪仍显示真实禁用原因。正式 WBP 交互与双机实际通话证据分层记录，模块整体验收仍未完成。
...
## 2026-09-17 Steam 右键加入失败后的菜单搜索恢复

用户在编辑器启动的独立游戏进程中复现。改前证据为 `Saved/Logs/Catfishing_2-backup-2026.09.17-03.27.17.log`：北京时间 11:24:09 出现 `FOnlineAsyncEventSteamInviteAccepted: Failed to parse connection URL`；11:24:17 打开加入页后出现 `Ignoring game search request while one is pending`，项目请求 `99C06BB9-4973-7711-D370-62B489732ED7` 一直保持 Find，11:27:11 仍只能往返菜单和加入页。UE 5.8 的 `OnlineSubsystemSteam/Source/Private/OnlineSessionAsyncServerSteam.cpp::FOnlineAsyncEventSteamInviteAccepted::Finalize` 在解析失败前建立 CurrentSessionSearch，却直接返回而不释放；`OnlineSessionInterfaceSteam.cpp::FindSessions` 在占用时返回 true，但不推进传入搜索对象。项目此前没有普通搜索截止时间，因此开始游戏和加入操作持续被 busy gate 拦截。

本次只修复这条已确认的永久等待与恢复链。平台收到的具体连接文本及其发布来源没有日志证据，不能声称右键加入成功连接已修复。保持原有 Lobby、密码/邀请准入、权限、地图旅行和存档规则。修改前已有 `Source/Catfishing/Fishing/Simulation/CatFishingFightSimulator.cpp` 和未跟踪裁决文档改动，不纳入本次提交；初始构建/测试未运行，既有设置页控件缺失与本修复分开记录。

| 功能/环节 | 当前位置与引用证据 | 现有行为与目标差异 | 处理方式与目标位置 | 衔接依赖与顺序 | 回归风险与验证方式 | 处理结果与证据 |
| --- | --- | --- | --- | --- | --- | --- |
| 入口、状态与时限 | `Source/Catfishing/Online/CatOnlineSubsystem.cpp::RequestFindSessions`，由 Frontend RoomModel 调用 | 原普通搜索无限等；目标固定 30 秒单调时间上限，独立于准入 20/30/35 秒 | 原类新增 SessionSearchDeadline，沿用 CoreTicker，不另建状态机 | 受理后固定期限，终态统一清零 | 正常成功/失败、未启动/无回调；JoinSearchInteraction | 已接入；运行结果见本节验证记录 |
| 平台取消与终态 | 同文件 `ExpireSessionSearch`、`TickPlatformInvites`、`ClearOperationDelegates`；原 Session 接口提供 CancelFindSessions | 原失败占用可遗留在 OSS；目标既解开 UI busy，也取消平台搜索 | 先解绑 Find 广播，保持 Find 状态调用取消，再结束为 NoSession/None 与 JoinTargetTimedOut，保留 RequestId | 不自动重试、不自动旅行；原终态推进 epoch；反初始化清期限 | 同步取消回调、迟到回调、平台重试；真实 NULL OSS 占用/取消回归 | 已接入；不存在第二套搜索入口 |
| 回执归属 | 同文件 `HandleFindSessionsComplete`；OSS 完成事件为共享广播 | 原仅校验 epoch/句柄，旧任务广播可能误结束新任务 | 还需本次 ActiveSearch 达到 Done/Failed；否则忽略，保留截止时间 | 完成时仍由原成功/失败方法收口 | 重试收到旧 epoch 和当前 epoch 的无关广播 | 已接入；未修改邀请、短码、好友定向查询的回执 |
| UI 与正式资产消费者 | `Source/Catfishing/UI/Frontend/CatFrontendPageController.cpp::RequestStartGameFlow`、`CatFrontendRootWidget.cpp::RefreshJoinPresentation`；`/Game/UI/Frontend/WBP_CatFrontendRoot`、`WBP_CatFrontendJoin` | 原 busy 永久拦截；目标失败后恢复加入控件与存档页导航 | 保留 Snapshot → RoomModel → Controller/WBP；复用现有 JoinTargetTimedOut 中文提示 | 后端发布终态后 UI 自动刷新，不强行解锁按钮 | 正式 WBP 受控回归；二进制图表额外消费者未确认，保留原接口 | 无资产修改；运行结果见验证记录 |
| 默认值、资源、权威、Cook 与生成脚本 | `Config/DefaultGame.ini` 的 CatOnlineSettings 与 ProjectPackagingSettings；`Scripts/upgrade_public_room_widgets.py`；`CatFrontendWidgetAuthoringLibrary` | 无需迁移资源、字段单位、Blueprint/WBP/DataAsset、动画、复制、存档或玩法权威 | 不涉及这些内容修改；保留既有 Frontend Cook 目录和原唯一准入入口 | 无生产资产切换、无新旧并行运行入口 | 最终 diff、配置与正式资产加载核查 | 保留；不把源码搜索未命中视为二进制无引用 |
| 日志、测试、旧路径与交付 | `LogCatOnline`；`Source/Catfishing/UI/Tests/CatJoinSearchInteractionTests.cpp`；本记录和唯一需求差距清单 | 原无搜索未启动/超时诊断；旧测试直接发完成广播且不建立搜索对象终态 | 新事件 online_search_not_started、online_search_timed_out、online_search_callback_unmatched；更新测试状态并保留原浏览返回契约 | 编译后按 contract/runtime_behavior/presentation_delivery 分层验收 | NULL 后端证据不得替代 Steam 双端；打包日志归 `<打包根目录>/Catfishing/Saved/Logs` | 测试旧假设已替换；没有废弃生产入口或资产需要删除 |

验证结果：

- `contract`：最终 `Saved/Logs/SteamJoinRecovery-EditorDelivery.log` 和 `SteamJoinRecovery-GameDelivery.log` 均为 `Result: Succeeded`。首次构建发现新增测试的 TSharedRef 与 delegate handle 类型用法错误，已更正；后续链接曾被运行中的编辑器 DLL 占用，按用户要求正常关闭窗口后已完成。原有编译器版本和 UE 弃用警告保留。
- `runtime_behavior`：`Saved/Automation/SteamJoinRecovery-20260917/Report/index.json` 的 LinkParsing、AdmissionTimeout、JoinSearchInteraction 三项通过，其中后两项含故意触发的失败路径 Warning。新增回归通过真实 NULL OSS 重现“已有搜索、返回 true、本次对象仍 NotStarted”，确认 CancelFindSessions 释放原搜索、同步取消时先解绑 Find、保持 busy 至统一终态、下一次生产 RequestFindSessions 真正进入 InProgress；受控对象另覆盖无回调、固定期限、旧 epoch/共享广播拒绝、输入保留和正式 WBP 按钮恢复及开始存档页可达。最终日志字段补齐后，`Saved/Automation/SteamJoinRecovery-Final-20260917/Report/index.json` 的 JoinSearchInteraction 再次通过（1 项含预期 Warning、0 failed）；同目录 Automation.log 确认 Actor/Authority/LocalRole/Player=Local 字段和 CancelAccepted=1，未输出账号或连接文本。
- `presentation_delivery`：同一组合中的 JoinPage 失败，错误均为改前日志已存在的 FrontendSettingsPage 九类控件缺失，本轮不修改或掩盖该独立资产缺口。JoinSearchInteraction 沿用已有的精确已知缺控件隔离，并通过正式 Root/Join/SaveList WBP 的受控交互；不将此证据表述为全前端表现或真实双端通过。没有资产布局变更、截图、新 Cook 或新包验收。
- 最终盘点：表内实现、清理与回执修复已完成；UI 沿原通知链恢复，超时提示复用原 `JoinTargetTimedOut` 文案，无新增文案/资产入口。新增 Deadline 只属于普通搜索，正常终态和 Deinitialize 均经 ClearOperationDelegates 清零。配置与生成/Cook 路径保留；搜索回归原来不设终态就直接广播完成的旧假设已移除，未留下第二套实现。未确认的二进制图表消费者保留现有接口，没有删除资产。

用户补充朋友可能不是同一版工程，不能据此推断地址异常一定由版本差异导致。尚需统一双方构建和已开启联机房间的前提，真实双账号重复 Steam 热启动右键加入及成功/失败后的重试，并检查两端默认落盘日志；本轮无新 Cook/打包或双端验收。日志过滤词为 `online_search_not_started`、`online_search_timed_out`、`online_search_callback_unmatched`；以 RequestId/epoch 核对超时和终态。Frontend / Online 模块不关闭。

## 2026-09-16 房间文字聊天

采用 Steam Lobby Chat，不新建 TCP 连接。准备房间的客户端还没有进入房主 UE World，因此不通过 PlayerController RPC 发送文字。`UCatRoomChatSubsystem` 以 GameInstance 生命周期持有消息；UI 只提交文字并消费只读列表。消息包为 `CATC` 标识、版本 1、16 字节消息 ID 和 UTF-8 正文，最多 200 个 Unicode 字符、1024 字节正文；不携带可自报的玩家身份、系统消息身份或玩法命令。发送者取 Steam 回调并复核当前 Lobby 成员。每人允许初始 3 条突发、每秒恢复 1 条，收发两端校验；接收回调队列最多 64 条，显示最近 100 条，去重窗口最多 512 项。同房换图保留历史，离房/换房清空；不补发入房前历史、不落盘正文。

修改前基线：工作区已有鱼竿组件、鱼竿/抓取测试及未跟踪裁决文档改动，随后还有库存职责链并行修改；均不纳入聊天提交。初始构建未运行，不能把首次构建失败当作旧基线。已知设置页控件缺失沿用上一节记录；本轮全前端字体巡检另外观察到旧 `WBP_CatFrontendLoading.LoadingDetailTextBlock` 字体不符，房间页字体通过，该加载页未修改。源码初版的成员名遮蔽和 Automation 枚举版本差异已修正。资产脚本首次在 commandlet 调用 PIE 状态查询造成失败，随后修正 commandlet 分支以及 Python 的文本对齐、输入框样式属性接口。默认 Zen 缓存服务不可达，验证进程使用引擎已有 `InstalledNoZenLocalFallback`，不修改工程缓存配置。

| 功能/环节 | 当前位置与引用证据 | 现有行为与目标差异 | 处理方式与目标位置 | 衔接依赖与顺序 | 回归风险与验证方式 | 处理结果与证据 |
| --- | --- | --- | --- | --- | --- | --- |
| 平台入口和身份 | `Source/Catfishing/Online/CatOnlineSubsystem.cpp::HandleJoinSessionComplete/GetSnapshot` 留在 Lobby；`CatRoomChatSubsystem::Initialize` 订阅 `OnSnapshotChanged` | 原先无文字传输；新增纯文本、成员身份核验，准备和开局契约不变 | `Source/Catfishing/Online/CatRoomChatProtocol.h`、`CatRoomChatSubsystem.h/.cpp`，调用 Steam `SendLobbyChatMsg/GetLobbyChatEntry` | Online 初始化 → 聊天订阅 → UI 绑定 | UTF-8、超长、无身份包、去重、限频；协议 Automation 与真实双账号 | 协议和本地状态实现；真实双账号收发待验收 |
| 回调、状态和清理 | 引擎 `OnlineAsyncTaskManagerSteam::OnlineTick` 在异步线程驱动 Steam 回调；项目 `CatSteamJoinLink.cpp` 使用加锁跨线程交接 | 新消息必须先复制 callback 作用域内数据，禁止工作线程访问 UObject/UI；同房换图保留，换房清空 | `FCatRoomChatSteamBridge::Configure/Enqueue/Drain` 有界加锁队列，CoreTicker 游戏线程消费；房间 Epoch 丢弃旧队列 | 初始化回调 → 当前房间过滤 → 游戏线程校验 → 通知；退出先解除 ticker/回调 | 异步投递、队列溢出、迟到、销毁；AsyncInbox 和 Lifecycle | 已实施；日志区分本地回传与远端接收，不宣称全员已读 |
| UI 正式资产 | `/Game/UI/Frontend/WBP_CatFrontendRoom`；`Scripts/style_frontend_room_scene.py` 原 `RoomChatSurface` | 两行占位改为折叠摘要、340×308 向上展开区、消息状态、输入和未读；不挤动底部动作 | `Scripts/style_frontend_room_chat.py::style` 保留原容器，新增具名控件；Root `.Chat.cpp::RenderRoomChat` | 接收方和渲染先编译 → 脚本生成保存 WBP → 实例画面检查 | 中文换行、长连续文本、滚动、底部按钮；正式 WBP 截图和字体引用 | 原占位 `RoomChatTitle/RoomChatUnavailable` 已移除；生成脚本自身检查重复生成控件集合相同 |
| 输入与调用消费者 | `CatFrontendRootWidget::BindPageControls/UnbindPageControls`、`NativeOnKeyDown`、`.RoomDialogs.cpp::NativeOnPreviewKeyDown` | Enter 聚焦/提交；失焦不发送；Esc 弹层→聊天→原退出；失败保留草稿 | Root 聊天实现绑定按钮及文本控件原生 `OnTextCommitted`；UI 不复制 Session 状态 | 保留弹层优先级，聊天收起后恢复 Root 焦点 | 意外离房、输入抢焦点、输入法；真实 Slate 键路由和中文输入法实机检查 | 普通键盘与 UI 检查已接；中文输入法候选词确认仍待人工实测 |
| 预览/检查工具 | `Source/CatfishingEditor/UI/CatFrontendWidgetAuthoringLibrary` 原负责 WBP 构造和预览 | 原截图会包含整个编辑器；增加独立 1280×720 PIE 窗口 | `.Preview.cpp::BeginFloatingFrontendPreview/PressFrontendPreviewKey`，复制临时 Play 设置，不保存用户配置；`Scripts/verify_room_chat_ui.py` | Editor-only 工具编译 → 正式 WBP 实例 → 截图后等待下一帧再换状态 | 错帧截图、尺寸、模拟消息冒充网络；分离 presentation 与网络证据 | 样例消息仅在检查脚本，独立窗口使用正式房间 WBP；不进入 Game 目标 |
| 默认值、持久化、Cook、文档和日志 | `Catfishing.Build.cs` 已有 Steam/UMG；`Config/DefaultGame.ini` 的 `/Script/UnrealEd.ProjectPackagingSettings` 收集 `/Game/UI/Frontend` | 不增端口、SaveGame 字段、资源写入、DataAsset、动画或玩法复制；限制值见协议 | 沿用正式 UI Cook 目录与中文字体；`LogCatOnline` 输出 `room_chat_*`，`LogCatUI` 输出控件缺失 | 源码 → WBP → 接口清单/本记录 → 唯一差距清单 | 打包日志、两端消息关联、资产依赖；构建和实际 Development 包联机 | 诊断含 World/NetMode/Authority/LocalRole、脱敏 Lobby/Member、MessageId、Result，不含正文；本轮未重新 Cook/打包 |

验证入口：`Saved/Logs/RoomChat-EditorBuild.log`、`RoomChat-GameBuild.log`；协议/生命周期/异步队列报告 `Saved/Automation/RoomChat/Contract/index.json`；资产生成 `Saved/Logs/RoomChat-Authoring.log`；正式 WBP 表现检查 `Saved/Automation/RoomChat/UI/checks.json` 与同目录截图，运行日志 `Saved/Logs/RoomChat-UI.log`。各层最终结果以本节收尾记录为准。

真实双端验收仍需两账号：在同一房间互发中文、连续短消息和 200 字边界，确认双方仅一条、名字正确；断线显示失败或未确认，换房不串消息；检查中文输入法候选词 Enter 不误发送；开局后返回同房记录保留。使用 Win64 Development 包，不加 `-log`，分别检查两端 `<打包根目录>/Catfishing/Saved/Logs` 的新日志，以相同 `MessageId` 对照 `room_chat_send_requested` 和 `room_chat_received`，单端 LocalEcho 不能证明双端成功。聊天 UI 当前只接准备房间，未新增局内 HUD 聊天入口。以上缺口仍归 Frontend / Online 模块，不关闭整模块。

本轮收尾结果：

- `contract`：最终 Editor 与 Win64 Development 构建通过；`RoomChat/Contract/index.json` 的 Protocol、Lifecycle、AsyncInbox 共 3/3 成功（2 clean、1 含预期超时/离线拒绝 Warning）。覆盖 UTF-8、版本/长度/控制字符拒绝、速率、身份加 ID 去重、回传不重复、超时与迟到回传、100 条上限、换房/离房，以及真实线程池入队与 64 条队列上限。编译期间一次库存 RPC 参数不匹配和一次 Editor 模块正在重链接造成的启动失败，均来自并行工作区的中间状态；接口同步和链接完成后复验成功，未改写该并行职责链。
- `runtime_behavior`：正式 WBP 检查 `RoomChat/UI/checks.json` 为 18 项 true、error=null。实际 Slate Enter 聚焦、离线发送拒绝保留草稿并显示原因、Esc 收起后仍停留房间、未读/滚动及禁用态通过。一次真实截图暴露长消息自动换行后未滚到底，最终使用明确内容宽度和布局预计算修复，并通过 `long_message_reaches_latest` 验证；不是修改断言期待来通过。此层仍不含真实 Steam 双端消息。
- `presentation_delivery`：正式 `WBP_CatFrontendRoom` 已保存，脚本重复生成的控件集合相同，旧占位已移除。已目视 1280×720 的 `Expanded00004.png`、`LongAndDeliveryStates00004.png` 及同轮折叠/未读/离线截图；面板收窄为 340 后不挡第一只猫和底部按钮，发送中/未确认提示、长中文换行、字体与正式字体包引用正常。房间成员姓名窄排属于原席位资产行为，本轮未改动该资产。未重新 Cook/打包、未核实真实 IME 和双账号收发，不宣称交付验收全闭合。
- 表内结果逐项落地：消息入口/线程/生命周期对应三组 Automation；正式 UI/输入/预览对应 18 项 WBP 检查；配置沿用已核对的 Steam 依赖和 Frontend Cook 目录，未新增持久化；接口清单与差距清单已更新。本节取代此前历史小节里的“聊天尚未开放”作为当前实现口径，当前剩余的是上述实机验收缺口，没有保留第二套聊天实现。


2026-09-17 语音 UI 补充：房间设置从静态“暂未开放”切换到本机已应用输入模式和当前可用状态，明确大厅需进入游戏后生效。局内右上角加入实际声音触发的玩家名与动态小喇叭；常开但无声不显示，远端需处于本机可听范围，禁用/松开 V/离场立即清除。使用现有语音采集及解码包络，不增加录音或复制链。正式 WBP/渲染/测试证据与真实 Steam 双机未验收边界见《主界面子技术文档》末节。
