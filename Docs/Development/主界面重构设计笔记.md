...
2026-09-17 最新人工决定：接入游戏分类的“麦克风设备”选择，原“暂不可用”许可仅继续适用于“语音输入模式”。主菜单和局内菜单读取同一设备 Model；选择、取消、恢复默认仅改草稿，应用后保存成功选择。当前实现限定 UE 5.8 Win64 Steam；实际双机收音验收与引擎设备限制见《主界面子技术文档》末尾，不把编译通过当作实机通话成功。
...
- 语音输入模式仍暂不可用；麦克风选择已按 2026-09-17 用户确认接入。正式 WBP 交互与双机实际通话证据分层记录，模块整体验收仍未完成。
