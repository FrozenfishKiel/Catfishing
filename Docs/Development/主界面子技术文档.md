...
历史基线是创建 Session 成功后立即进玩法图，之后已拆分创建与开始。2026-09-14 当前实现进一步改为：在现有 Frontend World 原地启动 UE Listen，确认成功后创建平台 Session，创建失败回滚监听；房主显式调用 `RequestStartHostedGame()` 后才预载并旅行。当前客户端仍通过 Steam Lobby 准备并等待玩法 ready；密码、邀请码、邀请免密以及前台 UE 准入迁移尚未完成，前台直接连接明确拒绝。阶段证据见《主界面重构设计笔记》“创建房间时启动 UE Listen Server”。
