...
- 用户确认口径（2026-09-07）：主界面不拆临时版本，按一次性完成的正式闭环设计；背景最终同时支持静态和动态；`加入队伍` 暂只保留按钮；设置分类为游戏、画面、声音、控制；存档页改为类似 Minecraft 的单页列表；Save 首批保存库存内容和角色位置；邀请码必须真实可用；加载页必须接异步加载进度；旧 `UCatTravelWidget` 和相关测试脚本口径删除。
...
├─ WBP_CatFrontendLoading
├─ WBP_CatSaveSlotRow
├─ WBP_CatRoomFriendRow
└─ WBP_CatRoomPlayerSlot
```

首页和退出确认属于同一个菜单业务面，`加入队伍` 当前只作为按钮保留，不单独拆页面。存档列表、房间、设置和加载有不同的数据来源或异步刷新节奏，可以独立为子 WBP。存档槽、好友行和玩家槽是动态列表条目，可以单独做条目 WBP。
...
- 持有菜单、存档列表、房间、设置和异步加载视图引用。
- 响应 `PageController` 的明确切页调用，例如 `ShowMenu()`、`ShowSaveList()`、`ShowRoom()`、`ShowFrontendSettings()`、`ShowLoading()`。
...
- `房主开始游戏`：要求 `RoomModel` 提交正式开始命令，随后切到异步加载视图。
...
加载页暂不单独建第四个 Model。它是 Controller 在读档、房主开始游戏和地图异步加载期间切出的 View。加载页需要的第几天、献祭进度、提示文本和加载进度，从 SaveModel、RoomModel、Save/Persistence 或异步加载链读取，不另建一份 Loading 数据包。

主界面任务必须顺带实现异步加载能力，加载页必须显示真实进度。不得写固定百分比或伪进度。
...
- 新增 `FrontendRootWidget`、`FrontendPageController`、三个 Model 的 transient 持有。
- 从 `UCatUISettings` 读取 `FrontendRootWidgetClass` 创建根 WBP。
- Online 快照变化时通知 `RoomModel`，RoomModel 变更后通知房间页刷新；需要进入或退出加载页时由 Controller 切 View。
...
`CatUISettings` 需要新增 `Frontend` 分类配置：
