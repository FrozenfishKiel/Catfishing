...
flowchart TD
    ...
    LP --> GameplayLoadingOverlay["Start Global Loading Overlay<br/>/Game/UI/Frontend/WBP_CatGameplayLoading"]
    LP --> ReturnLoadingOverlay["Leave Global Loading Overlay<br/>/Game/UI/Frontend/WBP_CatReturnToMainMenuLoading"]
...
├─ WBP_CatGameplayLoading  # 进入游戏全局遮罩内容，不嵌入 Root
├─ WBP_CatReturnToMainMenuLoading  # 回主菜单全局等待内容，不嵌入 Root
├─ WBP_CatSaveSlotRow
├─ WBP_CatRoomFriendRow
└─ WBP_CatRoomPlayerSlot
```

首页和退出确认属于同一个菜单业务面，`加入队伍` 当前只作为按钮保留，不单独拆页面。存档列表、房间和设置有不同的数据来源或异步刷新节奏，可以独立为子 WBP。进入游戏 Loading WBP 与回主菜单 Loading WBP 是 LocalPlayer UI 直接盖到视口最高层的两种全局遮罩内容，不属于 Root 的页面切换器，也不互相复用。存档槽、好友行和玩家槽是动态列表条目，可以单独做条目 WBP。
...
Loading 不再是 Frontend Root 的子页面，也不再是局内 ESC 菜单里的局部等待面板。`/Game/UI/Frontend/WBP_CatGameplayLoading` 承载进入游戏的全局进度遮罩，`/Game/UI/Frontend/WBP_CatReturnToMainMenuLoading` 承载回主菜单的全局等待遮罩；两者都由 `UCatLocalPlayerUISubsystem` 在 Online 快照进入 Start、Leave、TravelQueued、TravelingToLake 或 TravelingToFrontend 等等待阶段时按当前操作选择并加到最高层视口。
...
`CatUISettings` 保留 Frontend Root 配置，并分别声明进入游戏与回主菜单的全局 Loading WBP：

```text
FrontendRootWidgetClass = /Game/UI/Frontend/WBP_CatFrontendRoot.WBP_CatFrontendRoot_C
GameplayLoadingWidgetClass = /Game/UI/Frontend/WBP_CatGameplayLoading.WBP_CatGameplayLoading_C
ReturnToMainMenuLoadingWidgetClass = /Game/UI/Frontend/WBP_CatReturnToMainMenuLoading.WBP_CatReturnToMainMenuLoading_C
```
...
- Root、Controller、三 Model、LocalPlayer 接线和精确 Cook 配置已落地，旧 `CatTravelWidget` 源码与 LocalPlayer 专线已删；Frontend WBP 已按最新口径生成 Root、四个业务子页、进入游戏 Loading、回主菜单 Loading 和三个动态行资产，共 10 个 WBP。正式主界面仍缺完整端到端运行证据，不能仅凭资产存在视为完成。
