...
### 2026-09-18 返回主菜单局部回归

文档状态：待人工验收；存在测试缺口。本节只覆盖返回主菜单边界，不关闭 Frontend / Online 模块，也不把上述历史构建和遮罩回归结果算成本次验证。

事实来源：2026-09-18 用户要求只清理退出边界，并明确接受未确认图鉴记录丢失；当前工作区 `Source/Catfishing/Online/CatOnlineSubsystem.cpp` 的 `RequestLeave / BeginHostRunTeardown`、`Source/Catfishing/Framework/Game/CatfishingGameModeBase.cpp` 的 `RequestRunTeardown`、`Source/Catfishing/Framework/Game/CatfishingPlayerController.cpp` 的 `ClientPrepareForHostExit_Implementation`、`Source/Catfishing/UI/CatLocalPlayerUISubsystem.cpp` 的返回主菜单表现分支。验证事实来自主线程交接的 Editor / Win64 Development 构建结果、`Saved/Logs/ReturnMenu-Automation.log`、`Saved/Logs/ReturnMenu-LocalTravel.log`；最终单人回归日志为 `Saved/Logs/ReturnMenu-LocalTravel-Final.log`，记录 `Test Completed Result={Success}`；主线程已目视核查 `Saved/Screenshots/LocalRoom-Returned.png` 中主菜单根界面可见且无遮罩。以下人工步骤尚未取得人工通过结论。

本次目标是保留世界保存、玩法关闭、图鉴末次投递、会话清理与主菜单就绪判断，删除把未确认图鉴和远端退出回执作为退出门槛的等待。玩家仍从局内菜单选择返回主菜单；主菜单真正就绪后立即撤下遮罩，已移除退出完成态的固定展示停留，进入游戏表现不变。画面只显示保存、离开游戏或准备主菜单等玩家能理解的状态。尚未确认的图鉴记录允许随当前世界卸载而丢失，不能因此伪造图鉴成功确认；账号归属及图鉴持久化不在本轮修改范围内。

冷启动准备：使用包含本轮修改的 Win64 Development 包，从包内 `Catfishing.exe` 正常启动，不添加 `-log`。保存一份待测存档副本；单人用“开始游戏”选择该存档并进入游戏。联机用两台设备、两个 Steam 账号及同版本包，由房主开启联机并邀请客户端进入游戏。每轮记录点击返回主菜单与首页恢复可操作的实际时间，并保存双方各自的 `<打包根目录>/Catfishing/Saved/Logs`；本节不虚构秒数通过阈值，耗时仍需实机观察。

| 场景 | 操作与观察 | 通过标准 / 失败标准 | 失败时核查 |
| --- | --- | --- | --- |
| 单人手动保存后退出 | 进入单人游戏，打开局内菜单，手动保存并等成功提示，再选择返回主菜单；返回后重新进入同一存档 | 首页可操作，重进保留手动保存的已支持内容；若一直停留在退出遮罩、误报等待客户端或保存内容异常则失败 | 保存日志与 `run_teardown_ready` 顺序；区分保存失败、会话清理和主菜单未就绪 |
| 图鉴未确认仍退出 | 在已能复现图鉴写入拒绝的存档与账号条件下触发记录，再返回主菜单；不要通过改账号或伪造 ACK 制造通过 | 出现 `run_teardown_unconfirmed_grants` 警告后继续到 `run_teardown_ready` 和可操作首页；记录丢失属于已接受边界；因记录未确认继续等待或输出虚假写入成功则失败 | `LogCatRun` 中同一 RequestId/Epoch 的事件及图鉴拒绝日志；若无法形成未确认记录，本项记未验证 |
| 房主主动退出 | 双方进入游戏后房主返回主菜单，分别观察两端首页、输入和遮罩 | 房主不等待远端回执；客户端通过通知或断线收尾回主菜单；任一端卡住、崩溃或残留玩法输入则失败 | 双端日志；房主 `run_teardown_remote_exit_requested` 仅表示通知已提交，不能单独证明客户端退出成功 |
| 客户端独立退出 | 重新建立双人局，仅客户端选择返回主菜单，房主继续操作 | 客户端可操作首页、房主继续游戏；房主被一起退出、客户端要求房主图鉴清空或卡住则失败 | 客户端 Leave/会话清理链与房主成员离开日志；检查是否误走房主收尾 |
| 重复请求 | 在单人和联机退出开始后，尝试再次点击返回入口或重复按已绑定的菜单键；随后再进入并退出一次 | 没有重复保存/销毁副作用、崩溃和第二轮残留状态；若 UI 已屏蔽重复输入，只证明玩家入口受保护，内部幂等还需已有自动化证据 | `RequestLeave` 的操作状态检查及 `RequestRunTeardown` 的 `bTeardownComplete` 分支 |
| 退出画面 | 记录上述各场景的实际退出画面及主菜单恢复时刻 | 不出现 `Leave`、`DestroySession`、ACK、内部 gate 等诊断文案；不显示进入玩法的资源预载进度；主菜单真正可操作后解除遮罩 | `CatLocalPlayerUISubsystem.cpp` 返回分支与正式 Loading WBP；源码文本正确不能替代实际渲染检查 |

程序员阅读导航：先读 `CatOnlineSubsystem.cpp::RequestLeave` 的权限与保存分流，再读 `BeginHostRunTeardown` 如何消费同步结果；随后读 `CatfishingGameModeBase.cpp::RequestRunTeardown` 的关闭与末次投递顺序，并核对已完成分支不再重复收尾；最后沿 `ClientPrepareForHostExit_Implementation` 看客户端独立退出，以及 UI 返回分支是否仍等待真实主菜单就绪。上述文件均位于 `Source/Catfishing/` 对应 Online、Framework/Game、UI 目录。问题定位顺序是保存、权威收尾、会话销毁、前台旅行、遮罩解除，不能把每个阶段的停留都归因于加载。

分层验证记录（主线程交接）：

- `contract`：Editor Development 与 Win64 Development 游戏构建已通过。项目默认总检查失败，原因是 `Scripts/verify_delivery_harness_assets.ps1` 缺失；本节不据局部结果声称整体检查通过，也未绕过此失败。
- `runtime_behavior`：`Catfishing.Online.HostExit.NoAcknowledgementWait` 已通过，证据为 `Saved/Logs/ReturnMenu-Automation.log`；首轮真实单人菜单保存、退出及存档重读已通过，证据为 `Saved/Logs/ReturnMenu-LocalTravel.log`。首轮日志从 05:46:56.786 退出请求到 05:46:56.897 Online Frontend 成功约 0.11 秒，只代表该次旅行链路区间，不代表画面总耗时。最终加强回归已通过：`Saved/Logs/ReturnMenu-LocalTravel-Final.log` 记录 05:50:38.309 手动保存成功，38.312 退出开始，38.319 `run_teardown_unconfirmed_grants UnconfirmedGrants=1` 与 `run_teardown_ready`，39.089 遮罩隐藏，39.310 测试成功，包含存档重读、主菜单根可见及遮罩撤下断言。退出请求到撤罩约 0.78 秒，仅为本机本次非 Cook 的 `-game` 测试观测，不能推广为打包或联机耗时保证。
- `presentation_delivery`：未重新 Cook / 打包，未取得 Steam 双端退出、更新后打包画面、实际总耗时和双端默认落盘日志证据。`Saved/Screenshots/LocalRoom-Returned.png` 已由主线程目视核查，开始游戏等主菜单按钮可见、遮罩不存在；这是非 Cook 单人运行画面，不能替代打包或联机结论。

当前缺口为中等：人工步骤可执行，单人加强回归已通过，但打包表现与 Steam 双端证据未齐；默认总检查缺脚本还需恢复既有检查入口后重跑。未确认图鉴的自然触发条件仍需实机核对，受控注入不能证明账号归属问题已修复。反向验证重点为不重复收尾、不误退房主、不污染已保存世界及不提前解除遮罩；具体人工反馈仍待填写。

人工反馈回填：包版本 / 场景 / 实际耗时 / 是否通过 / 卡住阶段 / 双端日志与截图相对路径 / 是否需要继续修复，当前均待填写。
