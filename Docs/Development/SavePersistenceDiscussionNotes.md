...
更新时间：2026-09-15（v7 段随 `7f88ea4` 改；页头此前停在 09-11。2026-09-15 盘点：需求基线句仍指 08-31 飞书版本，现应指 `Knowledge/Design/`；「防偷保护牌」「偷鱼追回窗口」两处已随 09-11 裁决退役，待改）
...
- 正在进行中的过程状态不保存：钓鱼会话、客观拿鱼过程、供品结算预留、商店交易中间态、动作表现、镜头状态、临时输入锁和 UI pending 不进存档。读档只恢复存档中有的事实，不额外做一套“读档后取消”流程。偷鱼追回窗口已被 2026-09-14 用户裁决删除，不再作为可保存或待取消状态。
