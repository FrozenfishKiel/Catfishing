...
| 功能/环节 | 当前位置与引用证据 | 现有行为与目标差异 | 处理方式与目标位置 | 衔接依赖与顺序 | 回归风险与验证方式 | 处理结果与证据 |
| --- | --- | --- | --- | --- | --- | --- |
| 输入、疾跑、网络权威 | `Framework/Game/CatfishingPlayerController::Move/ApplySprintSpeed` → `PhysicalBody::SetMoveIntent`；`Character/CatCharacter::ConfigureCharacterMovementAuthority` | 原正式角色关闭 CMC Tick/移动复制并发自定义输入；目标本地预测且服务器仍唯一裁决。普通/疾跑默认 100/350 cm/s，继续读猫类与 Controller 配置，不接受客户端速度 | `CatCharacterMovementComponent::TickComponent/PerformMovement`、`CatCharacterNetworkPrediction` 保存输入；疾跑走 compressed flag；正式角色不再发旧移动/疾跑 RPC | 接收方/保存记录先就绪，再启用 CMC 并移除正式快照写位置 | 模拟延迟丢包、起停、疾跑保存记录、真实 Controller 输入 | 11 项 CMC 与两种正式猫真实 Controller 抓推通过，Final 报告；服务器不接受客户端速度/力量 |
| 核心运动、受力、体力 | `PhysicalBody::CaptureDriveSample`、`CMC::CalcVelocity`；Fishing/Grab → `AddExternalImpulseFromAuthority/SetExternalForceFromAuthority`；`PhysicalEffort::SettleMovementFromAuthority` | 保留有限力、4 kg 质量、cm/cm·s⁻¹/kg·cm·s⁻² 单位、重力、地形和体力计量。远端服务器由 CMC 时间戳移动驱动，房主/无 Controller 由 PostPhysics CMC Tick 驱动 | 原 `AdvanceFromAuthority` 迁移为 `PerformMovement`；保存权威受力策略用于客户端重放；冲量只在 authority 消费一次，预测不结算 ASC、不改变其他 Actor | PrePhysics 产力 → 唯一 CMC 移动入口 → authority 结算/模型最终分离；原 Body PostPhysics 不再推进第二步 | CMC 60/120Hz、卡顿、牵引、互推、体力与搏鱼用例 | CMC、PhysicalEffort、PhysicalRod 的 60/120Hz 与卡顿回归通过；模型停步为同提交既有失败；未实现完整 Chaos 世界回滚 |
| 移动后消费者顺序 | `Animation/CatForceReactionComponent::BeginPlay`、`Fishing/Integration/CatFishingPhysicalRodComponent::RefreshControlledCarrier` → `PhysicalBody::GetPostMovementTick` | 原接口保证力结算、身体移动后再处理动画与杆；不能在移除 Body 手工推进后仍返回旧 Tick | 保留公共接口；从宿主 CMC 返回实际 PrimaryComponentTick，组件 BeginPlay 早于身体初始化时也能正确绑定；原型仍返回原 PostPhysicsTick | Body PrePhysics 提交受力 → CMC → 受力动画/鱼竿/Character/网格；不修改并行任务的钓鱼源码 | 接口契约、受力动画三端、原杆端点、四人抓握/拾竿；检查无自身 Tick 依赖 | 公共接口已返回 CMC Tick；新增接口契约、受力动画三端、正式竿端点回归通过；三客户端整链仍有抓点保留缺口 |
| 跳跃与生命周期 | `PhysicalBody::RequestJump/ClearControlIntent/BeginControlEpochFromAuthority/TeleportBodyFromAuthority` | 跳跃 420 cm/s、短时抓握牵引 .35 s 不变；本地跳跃进入 saved flags，房主仍在力求解前直接调用 CMC DoJump | `CMC::DoJump/ResetControlPrediction`；网络移动携带 ControlEpoch，服务器拒绝旧周期；清理 saved/pending/acked move 时保留时间戳计时 | 控制周期先变，旧移动不得跨周期；传送强制权威校正 | 跳跃落地、迟到输入、失焦、传送、倒地、销毁 | 延迟丢包跳跃、传送旧输入隔离、落地、失焦、倒地/恢复和销毁回归通过；Final 报告 |
| 动画、手部、可见模型、镜头 | `PhysicalBody::GetVelocity/GetMoveIntent/IsGrounded` → Locomotion/ABP/UI；`PhysicalVisual::RefreshVisualPose`；`CatCharacter::CalcCamera` → 蓝图镜头/搏鱼镜头 | 原快照速度/支撑改读 CMC；可见 poseable mesh 需消费隐藏动画源的平滑偏移，同时保留自身配置变换；搏鱼输入朝向不能被自由镜头覆盖 | 只读接口及 `CompleteCharacterMovement`；可见模型应用源网格相对初始姿态的平滑增量；CMC 包的 ControlRotation 使用保存的物理瞄准 | 原始状态/手部 → 动画源 → 可见网格 → 模型接触；不修改 IK 公式、Montage、镜头资产 | 两种正式猫联机步态/抓爪、跳跃/倒地表现、实际截图 | 两正式猫步态/抓爪与跳跃/倒地运行回归通过；Rendered 4/4 通过，已检查实际步态、跳跃、倒地/恢复截图，局限见下文 |
| 配置、资产、生成与 Cook | `/Game/Character/BP_CatCharacterBase`、`BP_CatCharacter`、`BP_CuteCatCharacter`；`CatCharacterVariantAuthoringLibrary`、`Scripts/Art/create_character_family.py`、`finalize_character_family.py`；`Config/DefaultEngine.ini` SteamSockets | 保留 Blueprint 类、组件、缩放、速度和相机配置；无需变更资产路径、默认地图或 Steam 驱动 | C++ BeginPlay 重建胶囊后缓存正确 mesh offset 并统一启用 CMC；资产生产入口未发现需替换的移动逻辑 | 正式 Blueprint 实例验证；新旧包需使用同一编译协议 | Editor/Game 构建、正式资产运行；二进制隐藏消费者以运行实例为证，不能单靠搜索关闭 | 未修改资产/配置；FinalEditorBuild/FinalGameBuild 均成功；新 Cook/Steam 实机未验收 |
| 旧路径与诊断 | `PhysicalBody::TickComponent/OnRep_PhysicsSnapshot/ServerSetInput`；`LogCatPhysicsGrab` | 正式角色停止对快照位置 Lerp、停止手工覆盖 CMC 速度/支撑；原型仍实际调用旧通道，不能删除 | 移除 `AdvanceFromAuthority/ObserveSnapshot` 及快照驱动 IsFalling/IsMovingOnGround override；默认落盘 `cmc_prediction_started/sample/corrected`、`cmc_move_accepted/rejected`、`cmc_body_motion` | 状态变化日志立即写；运动/校正采样限频，静止不刷预测采样 | 双端 BodyId/ControlEpoch/MoveTime 检索、校正距离、未确认移动数 | 正式旧位置写入/手工推进已移除；实际日志含 cmc_prediction_started/sample/corrected、cmc_move_accepted/rejected；原型消费者保留 |
| 文档、测试、持久化 | 本文、唯一 `Docs/Development/需求对齐差距清单.md`；`Character/Tests/CatNetworkPredictionTests`、Editor `CatLocomotionNetworkTests` | 新增真实延迟/丢包网络验收和重放契约；历史证据保留日期，不作为当前完成声明。存档/库存/持久化写入不涉及 | 按 contract/runtime_behavior/presentation_delivery 记录实际结果 | 检查通过后独立中文 commit；不自动 push | 未运行层必须保留缺口，不关闭角色/Fishing 原子模块 | Final 共 56 项、54 通过、2 未通过；唯一需求清单记录缺口；未关闭角色/Fishing/Delivery 模块 |

模型接触衔接补充：`UCatModelContactComponent::RefreshPose` 直接消费 `PhysicsPrototypeVisualMesh` 的最终姿态用于权威碰撞。房主端保留原可见网格的实体变换，只有非权威客户端应用 CMC 隐藏网格的平滑增量；否则 ListenServer 的视觉延迟会进入碰撞/抓点裁决。保留同一组模型接触、力量、地形与 IK 参数，不新增第二套碰撞资产。

基线归因：`Saved/Validation/MovementBaseline-20260911` 从同一 `b955e0e` 构建，未叠加任何预测生产源码，仅给原模型测试补充测量。`Saved/Automation/MovementPrediction/SameBaseline` 的普通猫平地 60Hz 停步失败为速度已归零、模型分离仍推开 27.100cm；预测版本前轮测得 27.039cm，原 15cm 停止距离断言保留。基线三客户端原夹具还失败于搏鱼前抓点保留。更早的 `PeerPush-20260911` 副本缺少后来加入的 `CatForceReactionComponent`，其 `OldSlope` 成功伴有明确的缺类警告，不能用于本次因果对照。既有受力动画与模型分离缺口仍待单独修复，不以本次预测或编译成功关闭。

最终运行证据根为 `Saved/Automation/MovementPrediction/Final`：56 项中 39 clean、15 带诊断警告通过，2 failed、0 notRun。`contract`：`Saved/Logs/MovementPrediction-FinalEditorBuild.log`、`MovementPrediction-FinalGameBuild.log` 均成功；两个 SavedMove 契约和全部 CMC 运行用例通过。源码与隔离副本核对见同证据根上级的 `SourceManifest.json`：18份文件逐字节一致；三客户端夹具在并行提交 `82ac481` 中另增加了两行借竿视图断言，位于本轮失败阶段之后，未在该隔离报告中执行。本轮夹具的占有确认与真实接近改动与被测副本一致。

`runtime_behavior`：120ms 延迟、20ms 波动、5% 丢包下，客户端先于服务器起步/跳跃，停步后双方位置差小于3cm，旧移动不会撤销传送。正式普通猫/CuteCat 的 Controller 抓推/失焦、步态/双爪、跳跃/倒地恢复、三端受力事件、体力与原搏鱼端点用例通过。公共 Tick 衔接完成后的模型停步为 27.100cm、对方0.719cm，与同提交无预测基线逐项一致；速度均为0，额外位移来自模型分离。三客户端夹具保留全部旧行为断言，补了占有确认、真实移动接近、拾竿前落地/朝向等待；最终仍失败于架竿后助手身体握点保留。基线同一测试也未通过，但失败阶段是搏鱼前握点保留，不能声称完整三客户端链已验收或两者直接原因完全相同。

`presentation_delivery`：`Saved/Automation/MovementPrediction/Rendered` 的 D3D12 离屏运行 4/4 通过（均带诊断警告）。已查看隔离副本 `Saved/Automation/Locomotion/Images/20260911-104025-CuteWalkingIK.png`、`20260911-104035-FormalWalkingIK.png` 及 `Saved/Automation/PhysicalGrab/Images/20260911-104041-formal-jump-Jump.png`、`20260911-104052-formal-cmc-downed-animation.png`、`20260911-104100-formal-cmc-recovered-animation.png`：身体姿态对应行走、离地、趴倒和恢复站立。CuteCat 行走截图头顶被取景裁切，跳跃帧有运动模糊；截图仅支撑这些姿态观察，不能证明连续运动全程无顿挫。当前未新 Cook、未在 Steam 两台机器上复测，也未验收新 Development 包在不带 `-log` 时的房主/客户端独立落盘。最终自动化使用隔离副本端口18871，离屏检查用18872；仅修改该副本 Saved 下的编辑器用户设置。引擎已有 `-Multiprocess` 参数避免反复等待 SDK 查询，不修改项目打包配置。

Development 复测应在新包双方分别检查 `Catfishing/Saved/Logs` 的 `LogCatPhysicsGrab`：`physics_body_started Result=UprightCMCPrediction`、`cmc_prediction_started`、`cmc_prediction_sample`、`cmc_move_accepted`、`cmc_prediction_corrected`、`cmc_move_rejected`，按 BodyId/ControlEpoch/MoveTime 对齐。位移快照仍供诊断、策略与代理手部使用，正式角色不再拿它覆盖本地位置。房主和客户端必须使用相同的新 CMC 移动协议版本。
...
正式鱼竿当前通过选格拿到手，R 放杆、E 交互拾回、X 收包。R 放杆时由服务器将竿尾放到地面，恢复蓝图本地的上扬斜角并保留水平朝向；岸边竿尾下方无合适地面时尝试原持竿者脚下。落地后固定位置与角度、不自由下落，也不允许伸手抓取；放竿解除该竿全部手抓点。协作者只能抓住持竿角色传力，普通伸爪在手持和架放状态都不能抓竿；主控显式拿竿、抓人/互推/短时带起保持。其他原型动态道具仍按25%重力轻落。实现影响表及验证见[钓鱼架构中的放杆贴地修复](FishingArchitecture_zh-CN.md)。以下各日期章节保留对应历史证据，早期原位悬停/自由落竿/助手脱手后抓竿描述不再是当前规则。
...
- **runtime_behavior**：`Report-20260910-161133-433/index.json`为215项、214通过（191 clean、23 warning）、1既有耐久失败；之后仅补互抓4握点测试和观察镜头，`Report-20260910-161447-342/index.json`最终3/3通过（2 clean、1 warning）。实际日志对应`Automation-20260910-161133-433.log`与`Automation-20260910-161447-342.log`。默认`LogCatPhysicsGrab`记录`physics_body_jump`、`physics_body_external_lift`（历史日志名 `physics_body_grip_lift`）、`physics_body_jump_snapshot`、`physics_body_snapshot_observed`及`physics_grip_traction`；以World/NetMode、BodyId、GripId、ControlEpoch关联，不依赖Verbose。`GetJumpTractionWeight`是服务器窗口，客户端只消费姿态/速度/接地快照。
