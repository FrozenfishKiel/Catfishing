...
正式鱼竿当前通过选格拿到手，R 放杆、E 交互拾回、X 收包。R 放杆时由服务器将竿尾放到地面，恢复蓝图本地的上扬斜角并保留水平朝向；岸边竿尾下方无合适地面时尝试原持竿者脚下。落地后固定位置与角度、不自由下落，也不允许伸手抓取；放竿解除该竿全部手抓点。只有主控持竿时才允许抓竿协助，抓人/互推/短时带起保持。其他原型动态道具仍按25%重力轻落。实现影响表及验证见[钓鱼架构中的放杆贴地修复](FishingArchitecture_zh-CN.md)。以下各日期章节保留对应历史证据，早期原位悬停/自由落竿/助手脱手后抓竿描述不再是当前规则。
