...
2026-09-11 当前正式猫的辅助地面驱动已接入本人 ASC 力量和意图缺失体力结算，松方向键仍有限站稳，耗尽时主动出力为零，任何外部负载下禁止恢复。接触主动反力受各猫力量预算限制，30 N 只保留在被动碰撞部分；模型/骨骼数量不叠力。详见 [当前猫鱼/猫猫框架与验证](FishFightImplementationGuide_zh-CN.md)。下方带日期的原型和迁移说明为历史版本。
