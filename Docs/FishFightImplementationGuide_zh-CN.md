...
这份文档是鱼、线、杆、猫运动实现的阅读入口，解释当前源码怎样计算运动、负载和收尾。核对日期：2026-09-07。源码内可以搜索 `[FishLogic`，按 1～5 顺序阅读。
...
3. 猫的移动输入形成端点候选位移，与鱼候选和已放线长一起生成约束误差。两端当前只执行水平修正，因此先由线长球与鱼所在水平面的交线求出可行水平半径：`sqrt(max(L_paid² - 高差², 0))`，再按水平距离与该半径的差额分配修正；不能直接把三维超长量扣在 XY 上。只有鱼的向外对抗加速度高于猫时，其差值才按双方质量分配猫端牵引；猫更强或鱼力竭时，修正主要落到鱼端。修正仍受速度上限约束，高差超过线长时只收近 XY 并保留不可消除的高差，不凭空放线或抬鱼。相等力量下猫端牵引为零是这套公式的性质，不代表已经完成地面摩擦与合力求解。
