...
猫端沿绷紧鱼线把鱼拖向岸上则是另一种结果：只要竿尖位于岸线陆地方向，左键收线或猫/竿端移动确实产生向岸牵引，越岸候选不会再被内缩点挡回水里，而是立即进入与体力归零相同的 `ExhaustedReel/AutoHauling` 生命周期，停止鱼 AI 和鱼端驱动力并侧翻成鱼干。鱼干在水域轮廓内仍保持水面高度；第一次越过轮廓后记录为 `Beached`，此后每个固定步按当前 XY 从完整关卡高度垂直查询最高阻挡表面并更新 Z，所以连续拖过高岸、低洼或斜坡都不会继续沿旧水面高度钻地。拖到配置的竿尖完成距离后再生成可拾取 Pickup。鱼干阶段仍不结算猫端体力消耗。
