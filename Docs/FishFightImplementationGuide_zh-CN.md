...
活鱼不会因为自己的游动直接冲上陆地。水域查询发现候选位置越过岸线时，会给出岸线内侧安全点；若鱼只是自行撞岸，运动仍把修正拆成“向水内法向 + 沿岸切向”，保留沿岸滑动并由 `RedirectFromWaterBoundary()` 平滑游回水里。

猫端沿绷紧鱼线把鱼拖向岸上则是另一种结果：只要竿尖位于岸线陆地方向，左键收线或猫/竿端移动确实产生向岸牵引，并在越岸候选点找到高于水面的真实地面，就进入与体力归零相同的 `ExhaustedReel/AutoHauling` 生命周期，停止鱼 AI 和鱼端驱动力并侧翻成鱼干。力竭鱼不再需要有效游向，到达竿尖正下方也不会终止会话。水域烘焙轮廓与真实干地可能有间隙：力竭鱼在未确认干地前保持水面高度，真实拖拽继续保留本步进度，不弹回水域内缩点；纯甩杆不能触发上岸。确认真实干地后才记录为 `Beached`，后续每步按当前 XY 查询地面并更新 Z。干地鱼进入配置的竿尖水平完成距离后原地生成可拾取 Pickup，不再误用握把距离，也不要求交接帧仍按住左键。鱼干阶段仍不结算猫端体力消耗。
...
`LineLoad` 控制鱼线磨损和强对抗资格，但不再直接选择胜负；`Tension` 只表示几何约束介入，不能在鱼回头或横游时替代 `LineLoad` 制造满负载磨损。双方统一按沿线做功结算体力：Locked 绷紧时，鱼的向外意图同时形成猫的等长保持意图，所以即使实际位移接近 0，双方仍会消耗；Reeling 或主动后退已经提供猫的显式意图，不再重复叠加保持意图；FreeSpool 在 `L_max` 内确实解除约束时，鱼支付自己的自由游动做功，猫停止对抗并恢复体力；线放尽后重新形成张力，猫不再恢复。重大判定仍要求负载超过鱼性格阈值并持续确认时间，避免方向刚好扫过阈值一帧就断线。

鱼和猫先按 `加速度 = 有效力量 × AccelerationPerStrength` 生成意图速度，换算系数来自 `DA_FishingFightBalance_Default`；鱼性格游速和资产中的“收线速度”只做上限。猫移动只改变猫端候选位置，左键则独立缩短 `L_paid`，三者只形成一份线长约束误差。只有鱼的沿线加速度超过猫时，差值才按实际鱼重与猫等效质量生成猫端目标牵引；等力量自然僵持，猫占优或鱼力竭时修正全部落到鱼端。Rod 不把目标当作每步可累积的冲量，而是在服务器和拥有客户端的角色移动帧中平滑追赶；鱼力竭会立即清除平滑余量。收线请求不会因为鱼没能立即靠近而回填线长；原地收线与后退不会相互冒充或重复计算。
