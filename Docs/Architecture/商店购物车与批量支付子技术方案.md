...
文档状态：需求确认版。**2026-09-15 核：C++ 已接线**（`CatfishingPlayerController.h` 的 `ServerSubmitShopCartAtKiosk`、`ShopEconomy/CatShopCartCommandUtils.*`），09-13 去掉了钱包版本参数；文内「拟定 RPC」签名与价格表是当时样例，现行接口以 `Docs/Development/UI_WBP拼装接口清单.md` 为准；PIE 仍未见整局通过记录。
...
- `Source/Catfishing/ShopEconomy/Trading/CatShopTradeController.cpp` 已经按整车原子提交处理：先问营地公共仓库能否整批收货，再扣公款和货架库存，再发货并确认交付。
...
`ACatCampInventoryActor` 只提供宿主；当前整批预检与提交由其 `UCatInventoryComponent` 执行：
...
`FCatInventoryReceiveBatch` 的定义条目表达商店发货的定义和数量；校验时按公共仓库当前槽位预演整批堆叠和空槽占用，成功提交时写入正式库存条目、标记复制变更并广播。支付前置校验和真实入库使用同一组规则，不再传入或递增库存版本。

批量支付顺序：

1. PlayerController 校验来源摊位、玩家身份和摊位距离。
2. PlayerController 在当前 World 中解析 `ACatCampHubActor::ResolvePublicInventoryForShopOrder()`。
3. `UCatShopTradeController` 接收购物车命令。
4. `UCatShopEconomyService` 和商店交易入口解析当前目录、库存、总价和交付明细。
...
5. `Source/Catfishing/Framework/Game/CatfishingPlayerController.h/.cpp`：新增 `ServerSubmitShopCartAtKiosk` 和服务端转发函数，保持来源摊位距离证明和营地解析规则。
...
8. `Source/Catfishing/ShopEconomy/Trading/CatShopTradeController.h/.cpp`：提供购物车交易入口，串起付款前整批交付预检、付款、整批发货和交付确认。
