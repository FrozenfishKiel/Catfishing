...
`DisplayCategoryId` 是商品页分类，不写成 C++ enum，否则策划新增分类仍然要改程序；“全部”应是 UI 生成的虚拟页签，不要求每一行商品都填成全部。购买交付也不再依赖商店自己的商品类别枚举，商店只输出 `DefinitionId + PurchaseQuantity`，具体能否入库和如何堆叠由营地公共仓库与装备定义裁决。
