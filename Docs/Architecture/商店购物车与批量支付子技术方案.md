...
7. `Source/Catfishing/ShopEconomy/CatShopEconomyService.h/.cpp`：阅读 `PurchaseCatalogCart` 的终态重放、报价和整批容量检查、同步提交回调，以及成功后账本、缓存、库存通知和公开流水的顺序；付款故障继续追踪 `TryApplyTeamWalletTransaction`。
...
14. `Source/Catfishing/ShopEconomy/Tests/CatShopInventoryDeliveryTests.cpp`：阅读 `FCatShopInventoryDeliveryReplayTest::RunTest`，逐项区分购买服务断言与直接调用库存回调的断言；UI 结果恢复则回到 `CatShopPageController.cpp::HandleCampCommandResultReceived`。没有待交付确认 API 可作为第二次发货入口。
...
自动化入口为 `Catfishing.Unit.ShopEconomy.PurchaseImmediatelyStocksAndReplaysWithoutDuplicatePaymentOrItems`，由主 Agent 运行，结果统一引用 `Docs/Development/需求对齐差距清单.md` 的商店模块。该用例在独立 authority World 加载正式摊位蓝图、生成原生公共仓库 Actor，覆盖缺失仓库拒绝、直接库存提交回调拒绝、单条付费商品即时入库及首次广播时钱货账本状态、成功重放/同号改载荷及重放后广播次数仍为 1、原实例和堆叠恢复，以及实际填满格子和堆叠、断言待购批次无法接收并确认新货架报价与库存前提成立后的容量拒绝。满仓夹具已按实际填充修正，2026-09-14 主 Agent 复跑该用例为 Success；扩展 14 项中有 1 项已在 HEAD 复现的旧拾取断言失败，详见唯一进度入口。它没有通过正式营地蓝图、Controller 距离验证、RPC 网络、正式双端 WBP、真实钱包失败注入或多商品 UI 完成购买。

证据按三层分别记录：`contract` 为上述服务边界自动化（主 Agent 已运行，结果见唯一进度入口）；`runtime_behavior` 为真实权威成交、回滚、双端复制和回执（待验证）；`presentation_delivery` 为正式双端商店及仓库 UI 操作、提示、购物车恢复和内容一致性（待人工验收）。自动化通过也不能替代后两层。执行结果、证据路径和持续缺口统一引用 `Docs/Development/需求对齐差距清单.md` 的商店模块；本节只维护步骤与判定口径，不另建持续回填账本，没有执行证据的项目仍为待验证。
