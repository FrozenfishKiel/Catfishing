...
按本次确认的短握柄、细长软皮鞭概念制作。此目录保留 Blender 美术源资产；2026-09-18 已新增 UE 原型导入与 GAS 使用，当前接入说明见[物品使用与策划配置](../../../Docs/Architecture/物品使用与策划配置.md#皮鞭原型接入2026-09-18)。未配置商店出售或伤害行为。
...
- `AN_Whip_Idle.fbx`、`AN_Whip_Attack.fbx`：相同骨架与网格，分别烘焙一段动画。UE 已按仅动画方式导入并复用 `SK_Whip_Skeleton`；FBX Armature 变换节点使 UE 显示 18 个节点，Blender 变形骨骼仍为 17 根。
- `whip_manifest.json`：源资产统计与每帧鞭梢测量。

## 规格与编辑

Blender 5.2.2 LTS；米制；握柄约 0.2 m、鞭身约 1 m，总长 1.2 m。握柄中部是对象原点，参考姿态沿 +X 伸展，+Z 向上。UE 原型跟随猫的右前爪视觉骨骼位置，使用可调握点偏移；尺寸和握持仍待正式美术校准。
