...
| `Knowledge/Requirements/`  | 需求事实源地图；只保存飞书「小猫钓鱼」GDD 的权威入口和读取边界，不保存策划案副本。 | 写玩法逻辑、数值、交互规则和产品验收用例前，先按地图到飞书读取当前分册。 |
...
4. 看 `Saved/Automation/` 下最近一次运行的 `index.json` 与 `Automation.log`，确认当前实际验证过什么、还没验证什么。
...
测试分为 `Catfishing.Unit.*`（模块级合同）与 `Catfishing.Slice.*`（跨模块纵向切片）。实际条数与通过情况以运行后的 `index.json` 为准，不在文档里写死。
