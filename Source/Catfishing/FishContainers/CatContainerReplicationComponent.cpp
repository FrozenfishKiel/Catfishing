...
// authority 发布流程：验证 Owner 权威后替换 ID、类型、Revision 和容量元数据，再把每个鱼槽连同 SlotIndex 写入 FastArray；全部字段一致后广播只读变化并 ForceNetUpdate，终态缓存等私有事实不进入复制。
...
// 快照读取流程：返回组件持有的最终复制 DTO；调用方只能以 Revision 判断先后，不能把数组当写入口。
...
// 快照重建流程：覆盖容器 ID、类型、Revision 和容量元数据，再按复制项 SlotIndex 还原公开鱼槽；OwnerStableNetId 未复制且自然保持空。
...
// 合并安排流程：已有 pending 时直接复用；否则在有效 World 上登记下一帧回调。无 World 的异常复制入口退化为立即重建和广播，避免读模型永远停在上一值。
...
// FastArray 收包完成流程：UE 已应用本次新增、修改与删除，但元数据通知顺序仍不保证；因此只安排与 OnRep 共用的下一帧合并，参数不参与领域身份或快照序号选择。
