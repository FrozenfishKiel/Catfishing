...
/** 一局团队商店经济服务；只写公款、库存、订单和账本，不直接改 Equipment 或 Items。 */
UCLASS()
...
public:
	...
	/** WorldSubsystem 初始化时从显式 Settings 建立本局公款、库存和目录可用性。 */
	...
	/** 购买一条显式目录项；成功只扣公款、扣库存并写账本，具体交付定义由下游领域接续。 */
...
private:
	...
	/** 从 Settings 重建本局公款与库存；重复目录或非法目录项会让购买路径 fail-closed。 */
