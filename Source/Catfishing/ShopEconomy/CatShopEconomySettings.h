...
public:
	 ...
	 * 边界：不外推、不插值、不编造价格。飞书商店册 §3.1 只给了"体重重＝钱多"这个方向，没给任何斜率或截距，
	...
	/**
	 * 收鱼价体重轴是否已经被产品显式裁定。默认 Unset 表示"未裁定"，此时售鱼整体 fail-closed；
	 * 它和下面的档位表分成两个字段，是为了让"已裁定但当前档位表为空"和"未裁定"在运行期还能分开看。
	 */
