...
bool FCatShopEconomySettingsProjectDefaultsTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("项目默认免费普通饵入口"), Settings->FreeOrdinaryBaitEntryId, FName(TEXT("FreeBugBait")));
	...
	if (RodOrder)
	{
		...
		TestEqual(TEXT("二级鱼竿订单指向 Work1 装备定义"), RodOrder->DefinitionId, FName(TEXT("ShopRodT2")));
	...
	const FCatShopCatalogEntry* FreeBait = Settings->CatalogEntries.FindByPredicate(
		[](const FCatShopCatalogEntry& Entry)
		{
			return Entry.EntryId == FName(TEXT("FreeBugBait"));
		});
	...
	if (FreeBait)
	{
		...
		TestEqual(TEXT("免费虫虫饵指向 Work1 普通饵定义"), FreeBait->DefinitionId, FName(TEXT("BugBait")));
		TestEqual(TEXT("免费虫虫饵价格为 0"), FreeBait->UnitPrice, 0);
