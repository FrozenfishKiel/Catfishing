...
namespace CatShopEconomyServiceTest
{
	...
	struct FShopSettingsOverride
	{
		...
		/** 构造流程：保存默认对象全部可变字段，再写入测试显式 runtime gate 和基础公款。 */
	...
	/** 购买命令构造流程：填入服务器身份、公款版本和目录项；测试不提交价格或库存。 */
	...
	/** 交付确认命令构造流程：引用首次订单 TransactionId 和公款版本，再填入下游成功回执事实。 */
...
// 测试流程：配置一条有限库存装备订单和一条无限免费普通饵；购买必须扣公款/库存/账本并进入 Pending，交付确认只推进账本
...
bool FCatShopEconomyPurchaseAndFreeBaitTest::RunTest(const FString& Parameters)
{
	...
	if (TestTrue(TEXT("创建 ShopEconomy 购买测试 Game World"), WorldWrapper.CreateTestWorld(EWorldType::Game)))
	{
		...
		if (Service)
		{
			...
			TestEqual(TEXT("初始公款 Revision 为 1"), InitialWallet.Revision, static_cast<int64>(1));
			TestEqual(TEXT("初始公款余额来自配置"), InitialWallet.Balance, 10);
			...
			TestEqual(TEXT("购买扣除公款"), Purchase.Wallet.Balance, 7);
			TestEqual(TEXT("购买推进公款 Revision"), Purchase.Wallet.Revision, static_cast<int64>(2));
			...
			TestEqual(TEXT("账本记录公款负增量"), Purchase.Transaction.WalletDelta, -3);
			...
			TestEqual(TEXT("交付确认不改变公款余额"), Delivered.Wallet.Balance, 7);
			...
			TestEqual(TEXT("重复购买不二次扣公款"), Service->GetWalletSnapshot().Balance, 7);
			...
			TestEqual(TEXT("免费普通饵不改变公款余额"), FreeBait.Wallet.Balance, 7);
			TestEqual(TEXT("免费普通饵不推进公款 Revision"), FreeBait.Wallet.Revision, static_cast<int64>(2));
...
// 测试流程：先让购买、免费饵和售鱼用未来公款 Revision 首次失败，再推进真实公款到该 Revision；同 RequestId 重放必须命
...
bool FCatShopEconomyRejectedRequestReplayTest::RunTest(const FString& Parameters)
{
	...
	if (TestTrue(TEXT("创建 ShopEconomy 拒绝重放测试 Game World"), WorldWrapper.CreateTestWorld(EWorldType::Game)))
	{
		...
		if (Service)
		{
			...
			TestFalse(TEXT("未来公款 Revision 的购买首次不提交"), RejectedPurchase.Command.bCommitted);
			TestEqual(TEXT("未来公款 Revision 的购买返回 RevisionConflict"), RejectedPurchase.Command.Error,
			...
			TestTrue(TEXT("售鱼推进公款到购买原本等待的 Revision"), SaleToReachPurchaseRevision.Command.bCommitted);
			TestEqual(TEXT("公款已到达被拒购买的 ExpectedRevision"), Service->GetWalletSnapshot().Revision,
			...
			TestFalse(TEXT("被拒购买重放不因公款 Revision 变匹配而提交"), PurchaseReplay.Command.bCommitted);
			...
			TestFalse(TEXT("未来公款 Revision 的免费饵首次不提交"), RejectedFree.Command.bCommitted);
			TestEqual(TEXT("未来公款 Revision 的免费饵返回 RevisionConflict"), RejectedFree.Command.Error,
				ECatDomainCommandError::RevisionConflict);
			...
			const FCatShopTransactionResult SaleToReachFreeRevision = Service->ApplyFishSale(
				CatShopEconomyServiceTest::MakeFishSaleCommand(TEXT("PlayerA"), Service->GetWalletSnapshot().Revision,
					FGuid::NewGuid()));
			TestTrue(TEXT("售鱼推进公款到免费饵原本等待的 Revision"), SaleToReachFreeRevision.Command.bCommitted);
			const FCatShopTransactionResult FreeReplay = Service->ClaimFreeCatalogEntry(RejectedFreeCommand);
			TestFalse(TEXT("被拒免费饵重放不因公款 Revision 变匹配而提交"), FreeReplay.Command.bCommitted);
			...
			TestFalse(TEXT("未来公款 Revision 的售鱼首次不提交"), RejectedSale.Command.bCommitted);
			TestEqual(TEXT("未来公款 Revision 的售鱼返回 RevisionConflict"), RejectedSale.Command.Error,
				ECatDomainCommandError::RevisionConflict);

			const FCatShopTransactionResult SaleToReachRejectedSaleRevision = Service->ApplyFishSale(
				CatShopEconomyServiceTest::MakeFishSaleCommand(TEXT("PlayerA"), Service->GetWalletSnapshot().Revision,
					FGuid::NewGuid()));
			TestTrue(TEXT("售鱼推进公款到被拒售鱼原本等待的 Revision"), SaleToReachRejectedSaleRevision.Command.bCommitted);
			const FCatShopTransactionResult SaleReplay = Service->ApplyFishSale(RejectedSaleCommand);
			TestFalse(TEXT("被拒售鱼重放不因公款 Revision 变匹配而提交"), SaleReplay.Command.bCommitted);
			...
			TestEqual(TEXT("拒绝重放没有额外增加公款"), Service->GetWalletSnapshot().Balance, 25);
		}
		WorldWrapper.ForwardErrorMessages(this);
	}
	return !HasAnyErrors();
...
// 测试流程：售鱼入账必须带 Items 提交证据并按公款 Revision 并发；重复命令不二次加钱，同 RequestId 换提交证据会被拒绝，关闭后拒绝新售鱼。
bool FCatShopEconomyFishSaleAndCloseTest::RunTest(const FString& Parameters)
{
	...
	if (TestTrue(TEXT("创建 ShopEconomy 售鱼测试 Game World"), WorldWrapper.CreateTestWorld(EWorldType::Game)))
	{
		...
		if (Service)
		{
			...
			TestEqual(TEXT("售鱼增加公款余额"), Sale.Wallet.Balance, 5);
			TestEqual(TEXT("售鱼推进公款 Revision"), Sale.Wallet.Revision, static_cast<int64>(2));
			...
			const FCatShopTransactionResult Stale = Service->ApplyFishSale(
				CatShopEconomyServiceTest::MakeFishSaleCommand(TEXT("PlayerA"), InitialWallet.Revision,
					FGuid::NewGuid()));
			TestFalse(TEXT("陈旧公款 Revision 的售鱼不提交"), Stale.Command.bCommitted);
			TestEqual(TEXT("陈旧公款 Revision 返回 RevisionConflict"), Stale.Command.Error, ECatDomainCommandError::RevisionConflict);
...
// RequestId 仍能拿回首次终态，并且公款、库存、账本这些查询在收摊后照常可读——收摊冻结的是买卖，不是本局经济事实本身。
bool FCatShopEconomyClosedShopRejectsEveryWriteEntryTest::RunTest(const FString& Parameters)
{
	...
	if (TestTrue(TEXT("创建 ShopEconomy 收摊测试 Game World"), WorldWrapper.CreateTestWorld(EWorldType::Game)))
	{
		...
		if (Service)
		{
			...
			TestEqual(TEXT("收摊后公款余额不变"), Service->GetWalletSnapshot().Balance, WalletBeforeClose.Balance);
			TestEqual(TEXT("收摊后公款 Revision 不变"), Service->GetWalletSnapshot().Revision, WalletBeforeClose.Revision);
