 ...
 * 交付按目录条目的 Kind 分两条路：EquipmentGrant 进买家自己角色的装备槽，
 * RunConsumableGrant 进买家自己角色的一局耗材栈；二者都用订单 RequestId 当 Equipment 回执。
 *
 * 它存在的理由是《商店订单不拥有玩家装备》：商店只能记订单，买家 Equipment 只能改自己的槽位和耗材栈，两边都不该反过来调对方，
...
private:
	/**
	 * 声明：按同一条链跑完一笔订单，bFreeClaim 决定走购买还是免费自取写口，目录项的 Kind 决定交付落到本人装备槽还是角色耗材栈。
	 * 实现：先取本 World 的商店；再按目录项的 Kind 把交付侧的前提问在扣钱之前——耗材问收货人在不在、定义是不是
	 *       正式的一局耗材、这一栈还装不装得下；装备问买家 Equipment 在不在、定义是不是可装配装备。任一不成立就直接
	 *       返回，此时公款、商店库存和账本一个字都没动。前提都成立才提交订单，并只在订单确实成立时继续；已经交付过的订单
	 *       直接返回 AlreadyResolved；否则按 Kind 交付：装备类替换本人对应装备槽，耗材类授予 1 份一局数量栈，二者都以
