...
public:
	...
	void SetEquipmentDefinition(UCatEquipmentItemDefinition* InEquipmentDefinition);
	/** 当前掉落物的装备身份来源；生成器或蓝图可据此核对配置，但读取不会创建库存实例或提交拾取。 */
	UFUNCTION(BlueprintPure, Category = "Catfishing|Item")
	UCatEquipmentItemDefinition* GetEquipmentDefinition() const;
...
protected:
	...
	TObjectPtr<UCatEquipmentItemDefinition> EquipmentDefinition;
};
