...
bool ACatFishingSession::ScheduleWaitingProbeFromStateTree()
{
	...
	// Waiting 可以由“首次抛竿”或“入夜把已排队的试探期按空竿收回”进入。真咬窗漏按不再回到这里——
	// 它按 §3.4（:147）直接写鱼吐钩逃跑终局。入夜收回不释放鱼竿/鱼线/饵料预约，
	...
	// 每个咬钩机会使用独立、但可由服务器抛竿种子重放的随机流。否则入夜收回后重回 Waiting，
	...
	if (const UCatEquipmentDefinition* Bait = GetDefault<UCatInventorySettings>()->FindRuntimeDefinition<UCatEquipmentDefinition>(CastEquipment.IsValid()
...
bool ACatFishingSession::OpenTrueBiteWindowFromAuthority()
{
	...
	{
		UE_LOG(LogCatFishing, Warning,
			TEXT("Event=fishing_bite_timing_rejected SessionId=%s Fish=%s RarityTierId=%s Window=TrueBite Result=InvalidDuration World=%s NetMode=%d Authority=1 LocalRole=%d"),
			*Snapshot.FishingSessionId.ToString(), *Snapshot.FishDefinitionId.ToString(),
			FishDefinition ? *FishDefinition->RarityTierId.ToString() : TEXT("None"),
			*GetNameSafe(World), int32(GetNetMode()), int32(GetLocalRole()));
		return false;
	}
	UCatEquipmentComponent* Equipment = CastEquipment.Get();
	...
	// 墓碑（2026-09-14，T13/T11；钓鱼规则 §4.5:209）：只在真咬读取 D0，响应窗移动不重算。
	...
	const UCatEquipmentFragment_Rod* BiteRodFragment = BiteRod ? BiteRod->FindFragment<UCatEquipmentFragment_Rod>() : nullptr;
	TrueBiteDistanceCentimeters = FisherCharacter.IsValid()
		? FVector::Distance(FisherCharacter->GetActorLocation(), Snapshot.FishEncounterActor->GetFishingCollisionCenter()) : -1.0;
	if (!BiteRodFragment || !FMath::IsFinite(TrueBiteDistanceCentimeters) || TrueBiteDistanceCentimeters < 0.0)
	{
		FinalizeSession(ECatFishingPhase::Terminated, ECatFishingOutcome::Invalidated, TEXT("True bite distance unavailable"));
		return false;
	}
	UE_LOG(LogCatFishing, Log, TEXT("Event=fishing_true_bite_distance SessionId=%s D0Cm=%.3f LmaxCm=%.3f World=%s NetMode=%d Authority=1 LocalRole=%d"),
		*Snapshot.FishingSessionId.ToString(), TrueBiteDistanceCentimeters, BiteRodFragment->MaximumLineLengthCentimeters,
		*GetNameSafe(World), int32(GetNetMode()), int32(GetLocalRole()));
	if (TrueBiteDistanceCentimeters > BiteRodFragment->MaximumLineLengthCentimeters)
	{
		// 饵已提交，漂不丢；不能 Clamp 或先应用完美乘数把超长 D0 救回合法搏斗。
	...
	UE_LOG(LogCatFishing, Log, TEXT("Event=fishing_windows_resolved SessionId=%s Fish=%s ConfiguredProbeSeconds=%.3f ResponseSeconds=%.3f PerfectSeconds=%.3f RarityTierId=%s ResponseSource=%s World=%s NetMode=%d Authority=1 LocalRole=%d"),
		*Snapshot.FishingSessionId.ToString(), *FishDefinition->FishDefinitionId.ToString(), FishDefinition->ProbeDurationSeconds,
		ResponseSeconds, Snapshot.PerfectWindowEndsServerTime - World->GetTimeSeconds(), *FishDefinition->RarityTierId.ToString(),
		ResponseSource, *GetNameSafe(World), GetNetMode(), GetLocalRole());
	UE_CLOG(FCString::Strcmp(ResponseSource, TEXT("LegacyFallback")) == 0, LogCatFishing, Warning,
		TEXT("Event=fish_response_window_unconfigured SessionId=%s Fish=%s RarityTierId=%s FallbackSeconds=%.3f Reason=MissingAssetAndRarityDefault World=%s NetMode=%d Authority=1 LocalRole=%d"),
		*Snapshot.FishingSessionId.ToString(), *FishDefinition->FishDefinitionId.ToString(), *FishDefinition->RarityTierId.ToString(),
		ResponseSeconds, *GetNameSafe(World), int32(GetNetMode()), int32(GetLocalRole()));
...
FCatFishSelectionCommitResult ACatFishingSession::ResolveHookSelectionFromAuthority()
{
	...
	// 墓碑（2026-09-14，T13；钓鱼规则 §4.5）：试探期距离只供表现，不能成为入战 L0。
	const double InitialLineLength = FVector::Distance(FisherCharacter->GetActorLocation(), FishLocation);
	if (!Encounter)
	{
		SelectionResolution = ECatFishSelectionResolution::Failed;
...
{
	// 钓鱼规则 §3.4 提竿结果表（:147）："响应窗结束仍未提竿，鱼吐钩逃跑，饵已消耗"，
	// 再加 §3.3（:133）"进入咬钩后，无论上鱼、超时、放弃、断竿、落水，都消耗 1 份饵"。
	// 所以窗口到点是本竿的终局，不再回 Waiting 重新派生种子重新计时。
	...
	const FCatFishingUseOperationResult BaitCommit = Equipment
		? Equipment->CommitFishingBaitDeferred(Snapshot.FishingSessionId) : FCatFishingUseOperationResult{};
	const bool bBaitConsumed = Equipment
		&& (BaitCommit.bApplied || BaitCommit.Error == ECatDomainCommandError::AlreadyResolved);
	UE_LOG(LogCatFishing, Log,
		TEXT("Event=fishing_bite_opportunity_expired SessionId=%s Opportunity=%u BaitConsumed=%s BaitError=%s Result=FishSpatOutHook"),
		*Snapshot.FishingSessionId.ToString(EGuidFormats::DigitsWithHyphens), BiteOpportunitySequence,
		bBaitConsumed ? TEXT("true") : TEXT("false"), *UEnum::GetValueAsString(BaitCommit.Error));
	if (!bBaitConsumed)
	{
		FinalizeSession(ECatFishingPhase::Terminated, ECatFishingOutcome::Invalidated,
			TEXT("Hook window bait commit failed"));
		return;
	}
	TerminateSession(ECatFishingOutcome::HookWindowExpired, TEXT("True bite window expired; fish spat out the hook"));
}
...
bool ACatFishingSession::TryEnterHookedFightFromAuthority()
{
	...
	{
		// 求解/吸附/表现应用任一环节失败：直接不进入搏斗。入场不再补体力，所以没有体力状态要回滚。
		return false;
	}
	...
	{
		// Runner 创建/初始化/启动任一步失败：清空引用，不留下半启动的 Runner。体力没有被入场改写，无需回滚。
		FightRunner = nullptr;
		return false;
	}
	Snapshot.bReeling = FightRunner->GetCatAction() == ECatFightCatAction::Pull;
