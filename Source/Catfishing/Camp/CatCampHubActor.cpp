...
// 休息流程：现取本人 Character 并验证固定范围；随后只调用 Condition 的 CampRest 写口，营地不保存第二份 Fatigue。
...
// 篝火回看流程：先由服务器 UniqueId 与 RequestId 重放首次终态，再拒绝不在固定营地的请求者。若当前是结算夜且配置了封面事件，则逐个验证 GameState 当前玩家仍有有效身份、Controller 和营地内 Character，提交一份全员 Candidate，并通过批量接口先建齐所有 Planned 记录、再尝试逐人投递；任何预检或落盘失败都缓存失败且不广播，因此不会留下“部分玩家没有计划”的成功回放，离线造成的未投递则由同一记录重试。全部必需事实成立后只广播一次可跳过的表现意图并缓存成功，不写 next-day ready，也不等待客户端播放完成。
FCatDomainCommandResult ACatCampHubActor::RequestCampfirePlayback(AController* RequestingController, const FGuid RequestId)
{
	...
	const UCatCampSettings* Settings = GetDefault<UCatCampSettings>();
	const bool bSettlementNight = GameState
