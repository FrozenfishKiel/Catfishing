...
public:
	...
	bool CanFullyAcceptInventoryBatch(const FCatInventoryReceiveBatch& ReceiveBatch, int32 MinimumSlotCount = 0) const;
	...
	/** 组合事务暂缓观察通知；库存事实仍立即提交，最外层收口后读者才看到完整结果。 */
	void BeginDeferredInventoryPublication();
	void EndDeferredInventoryPublication();
	/** 退出托管直接移动活动区的同一 UObject；全部预检通过后才改两端，无中途广播。 */
	bool MoveHeldInventoryEntriesToCustodianFromAuthority(UCatInventoryComponent* Target,
...
protected:
	...
	bool SimulateAddInventoryBatch(const FCatInventoryReceiveBatch& ReceiveBatch,
		TArray<FSimulatedInventorySlot>& SimulatedSlots, int32 MinimumSlotCount = 0) const;
	...
	int32 DeferredPublicationDepth = 0;
	bool bDeferredPublicationPending = false;
