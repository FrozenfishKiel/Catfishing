 ...
 * 鱼表数据的蓝图只读入口。
 *
 * 存在的理由：表现层（鱼缸里游动的鱼、浮漂漂讯、水面特效）需要按 ItemId 拿到鱼种与表现资产，
...
public:
	...
	static UCatFishDefinition* FindFishDefinition(int32  ItemId);

	/** 按稳定 ID 取该鱼种的表现定义（网格／AnimBP／四段动画／缩放／Transform／漂讯与水面槽位）。 */
	UFUNCTION(BlueprintPure, Category = "Catfishing|Fish")
	static UCatFishPresentationDefinition* FindFishPresentationDefinition(int32  ItemId);
};
