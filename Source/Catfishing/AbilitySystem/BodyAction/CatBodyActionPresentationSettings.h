...
{
	GENERATED_BODY()
	/** 这条配置代表某个保留 BodyAction 事件的身份；配置资产写入，专用 Ability 及其表现任务只读取同一条记录。 */
	...
	/** 能力期间持续的可选音效或特效；GA 经标准 Cue 复制授予并在结束时回收，留空表示无附加表现。 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Catfishing|BodyAction", meta=(Categories="GameplayCue")) FGameplayTag GameplayCue;
	/** 可选正式 Montage；配置资产写入、GA 的蒙太奇任务读取；留空时不播放动画，前摇提交时点保持不变。 */
...
public:
	...
	/** 按 BodyActionEventTag 索引的表现配置；只描述保留 BodyAction 的前摇、GameplayCue 和 Montage，不承载物品事务或 Wet 状态。 */
