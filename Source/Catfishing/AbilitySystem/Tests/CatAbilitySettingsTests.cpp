...
// 测试流程：创建瞬态 Settings 对象，直接读取正式 readiness 和数值接口；默认配置必须整体拒绝运行，并把所有输出清成安全值。
bool FCatAbilitySettingsDefaultsTest::RunTest(const FString& Parameters)
{
	...
	TestFalse(TEXT("默认初始属性读取失败"), Settings->TryGetInitialAttributes(Hunger, Fatigue, Poison, FishingStrength, FightStamina));
	TestEqual(TEXT("失败时 Hunger 输出被清零"), Hunger, 0.0f);
	TestEqual(TEXT("失败时 Fatigue 输出被清零"), Fatigue, 0.0f);
	...
	TestFalse(TEXT("默认诊断 Hunger 改变量读取失败"), Settings->TryGetDiagnosticHungerDelta(DiagnosticDelta));
...
bool FCatAbilitySettingsValidValuesTest::RunTest(const FString& Parameters)
{
	...
	Settings->InitialHunger = 10.0f;
	Settings->InitialFatigue = 20.0f;
	...
	Settings->DiagnosticHungerDelta = -3.5f;
	...
	TestTrue(TEXT("完整初始属性配置可读取"), Settings->TryGetInitialAttributes(Hunger, Fatigue, Poison, FishingStrength, FightStamina));
	TestEqual(TEXT("Hunger 读取配置值"), Hunger, 10.0f);
	TestEqual(TEXT("Fatigue 读取配置值"), Fatigue, 20.0f);
	...
	TestTrue(TEXT("非零诊断 Hunger 改变量可读取"), Settings->TryGetDiagnosticHungerDelta(DiagnosticDelta));
	TestEqual(TEXT("诊断 Hunger 改变量保持配置值"), DiagnosticDelta, -3.5f);
	return !HasAnyErrors();
}
...
bool FCatAbilitySettingsPartialValuesTest::RunTest(const FString& Parameters)
{
	...
	TestFalse(TEXT("FightStamina 未配置时初始属性整体失败"),
		Settings->TryGetInitialAttributes(Hunger, Fatigue, Poison, FishingStrength, FightStamina));
	TestEqual(TEXT("半配置失败清空 Hunger"), Hunger, 0.0f);
	TestEqual(TEXT("半配置失败清空 Fatigue"), Fatigue, 0.0f);
