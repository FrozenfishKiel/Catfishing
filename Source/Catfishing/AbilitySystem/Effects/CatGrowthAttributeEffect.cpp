...
UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Data_Condition_YellowFightStaminaDelta, "Cat.Data.Condition.YellowFightStaminaDelta");
...
FGameplayTag UCatGE_YellowFightStaminaDelta::GetYellowFightStaminaDeltaTag()
{
	return TAG_Data_Condition_YellowFightStaminaDelta;
}

// 构造流程：Instant GE，把 SetByCaller 幅度以 Additive 方式作用到黄色体力护盾段。
UCatGE_YellowFightStaminaDelta::UCatGE_YellowFightStaminaDelta()
{
	...
	Modifier.ModifierOp = EGameplayModOp::Additive;
	FSetByCallerFloat SetByCaller;
	SetByCaller.DataTag = GetYellowFightStaminaDeltaTag();
	Modifier.ModifierMagnitude = FGameplayEffectModifierMagnitude(SetByCaller);
}
