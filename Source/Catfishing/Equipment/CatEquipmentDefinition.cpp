...
// 定义检查流程：验证总 gate、身份、类别与功能路线；装配类要求槽位，Rod 要正耐久，特殊饵必须是局内耗材，普通饵允许配置成有限或无限，Chum 还必须给出服务器读取的三轴增量。
bool UCatEquipmentDefinition::IsRuntimeDefinitionReady() const
{
	...
	{
		return FMath::IsNearlyZero(MaximumRodDurability) && (!bSpecialBait || bRunConsumable)
