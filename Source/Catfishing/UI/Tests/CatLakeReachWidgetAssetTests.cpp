...
// 测试流程：
// 1. 先加载现有 WBP_CatLakeReach；缺失时用 WidgetBlueprintFactory 创建继承 CatLakeReachWidget 的正式资产。
// 2. 复用已有资产时清空旧 WidgetTree 源控件和变量 GUID，再重建唯一 LakeReachRoot，避免旧白盒或半生成控件继续留在蓝图里。
// 3. HUD、Fishing 反馈、鱼护选择、鱼护动作、图鉴和菜单按钮全部挂在同一个 WBP 根内，避免把单一 UIReach 模块拆成多个业务交付项。
// 4. 给这些控件写入 Designer 属性绑定，让 WBP 读取 C++ View 的只读标量、结果和 Visibility 投影，正式表现不再靠 C++ 默认 SetText。
// 5. 最后编译并保存包；编译 ensure、保存失败、父类错误或绑定缺失都会让本测试失败，不能把脏资产当作正式前端证据。
bool FCatLakeReachWidgetFormalWBPAssetCreationTest::RunTest(const FString& Parameters)
{
	...
	TestNotNull(TEXT("创建标题文本"), AddVerticalText(
		MainStack, TEXT("LakeReachTitle"), TEXT("CATFISHING LAKE"), false, 24,
		FLinearColor(0.92f, 0.98f, 1.0f, 1.0f), FMargin(0.0f, 0.0f, 0.0f, 4.0f)));
	TestNotNull(TEXT("创建副标题文本"), AddVerticalText(
		MainStack, TEXT("LakeReachSubtitle"), TEXT("Run status, catch bag, and collection"), false, 12,
	...
	MenuStack->AddChildToVerticalBox(SelectionRow);
	...
	TestNotNull(TEXT("创建上一条鱼按钮"), AddButton(
		SelectionRow, TEXT("PreviousFishGuardButton"), TEXT("Prev"), FMargin(0.0f, 0.0f, 6.0f, 0.0f)));
	TestNotNull(TEXT("创建下一条鱼按钮"), AddButton(
		SelectionRow, TEXT("NextFishGuardButton"), TEXT("Next"), FMargin(6.0f, 0.0f, 0.0f, 0.0f)));
	AddSpacer(MenuStack, TEXT("SelectionActionSpacer"), 8.0f);
	...
	if (UVerticalBoxSlot* ActionPanelSlot = MenuStack->AddChildToVerticalBox(FishGuardActionPanel))
	{
	...
	TestNotNull(TEXT("创建动作区标题"), AddVerticalText(
		ActionStack, TEXT("FishGuardActionTitle"), TEXT("Selected Fish Actions"), false, 12,
	...
	TestNotNull(TEXT("创建吃鱼按钮"), AddButton(
		ActionRow, TEXT("ConsumeFishButton"), TEXT("Eat"), FMargin(0.0f, 0.0f, 5.0f, 0.0f)));
	TestNotNull(TEXT("创建转缸按钮"), AddButton(
		ActionRow, TEXT("TransferFishToTankButton"), TEXT("Tank"), FMargin(5.0f, 0.0f, 5.0f, 0.0f)));
	TestNotNull(TEXT("创建献祭按钮"), AddButton(
		ActionRow, TEXT("SacrificeFishButton"), TEXT("Sacrifice"), FMargin(5.0f, 0.0f, 0.0f, 0.0f)));

	TestNotNull(TEXT("创建鱼护反馈绑定文本"), AddVerticalText(
		MenuStack, TEXT("FishGuardResultText"), TEXT("Fish guard result"), true, 13,
		FLinearColor(0.94f, 0.88f, 0.80f, 1.0f), FMargin(0.0f, 0.0f, 0.0f, 8.0f)));
	TestNotNull(TEXT("创建图鉴绑定文本"), AddVerticalText(
		MenuStack, TEXT("FishCollectionText"), TEXT("Fish collection"), true, 13,
	...
	TestNotNull(TEXT("创建离局按钮"), AddButton(
		FooterRow, TEXT("LeaveButton"), TEXT("Leave Run"), FMargin(0.0f, 0.0f, 6.0f, 0.0f)));
	TestNotNull(TEXT("创建关闭按钮"), AddButton(
		FooterRow, TEXT("CloseButton"), TEXT("Close"), FMargin(6.0f, 0.0f, 0.0f, 0.0f)));
	TestTrue(TEXT("WBP 绑定 Poison 展示值"), AddWidgetPropertyBinding(TEXT("SurvivalText"), TEXT("Text"), TEXT("BlueprintPoisonValue")));
	TestTrue(TEXT("WBP 绑定 FishingStrength 展示值"), AddWidgetPropertyBinding(TEXT("FishingText"), TEXT("Text"), TEXT("BlueprintFishingStrengthValue")));
	TestTrue(TEXT("WBP 绑定 FightStamina 展示值"), AddWidgetPropertyBinding(TEXT("FeedbackText"), TEXT("Text"), TEXT("BlueprintFightStaminaValue")));
	TestTrue(TEXT("WBP 绑定个人鱼护数量"), AddWidgetPropertyBinding(TEXT("FishGuardText"), TEXT("Text"), TEXT("BlueprintFishGuardCount")));
	...
	TestTrue(TEXT("WBP 绑定图鉴数量"), AddWidgetPropertyBinding(TEXT("FishCollectionText"), TEXT("Text"), TEXT("BlueprintFishCollectionCount")));
	...
	TestEqual(TEXT("正式 WBP 前端拥有 Designer 属性绑定"), WidgetBlueprint->Bindings.Num(), 16);
	...
	if (bSaved)
	{
		UE_LOG(LogTemp, Display, TEXT("CREATE_UI_REACH_WBP_PASS Asset=%s RootClass=%s WidgetCount=%d BindingCount=%d FrontendPanel=LakeReachRootFrame ActionPanel=FishGuardActionPanel"),
			*ObjectPath,
