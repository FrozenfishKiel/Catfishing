...
// 3. 真实 Start 预载或预载完成后的旅行交给全局遮罩表现，Controller 只刷新房间反馈和 RequestId 去重日志。
...
void UCatFrontendPageController::HandleRoomModelChanged()
{
	...
	if (bGameplayStartInProgress && bCanShowGameplayLoading)
	{
		...
		Root->RefreshFlowFeedback();
		if (Snapshot.RequestId != PresentedGameplayLoadRequestId)
		{
			PresentedGameplayLoadRequestId = Snapshot.RequestId;
			UE_LOG(LogCatUI, Log, TEXT("Event=frontend_gameplay_loading_covered RequestId=%s Epoch=%lld World=%s NetMode=%d"),
				*Snapshot.RequestId.ToString(EGuidFormats::DigitsWithHyphens), Snapshot.OperationEpoch,
