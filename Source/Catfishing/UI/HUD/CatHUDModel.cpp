...
void UCatHUDModel::Refresh()
{
	...
	if (const UCatPhysicsGrabComponent* Grab = Body ? Body->GetGrab() : nullptr)
	{
		...
		// 手部投影只描述正在持竿、抓握或伸手的事实；按键映射不属于 HUD 状态。
		const auto HandLabel = [&](const bool bLeft, const bool bReaching, const bool bGripped)
		{
			if (bGripped && Grab->GetGripState(bLeft).bExplicitHold && NewState.bPrimaryRodOperator && Grab->GetGripTarget(bLeft) == Rod)
				return TEXT("持竿");
			return bGripped ? TEXT("抓住") : (bReaching ? TEXT("伸手中") : TEXT("收回"));
		};
	...
	NewState.CatStaminaText = FText::FromString(NewState.YellowFightStamina > 0.0f
		? FString::Printf(TEXT("玩家体力 %.0f / %.0f（黄色储备 %.0f）"), TotalStamina, TotalCapacity, NewState.YellowFightStamina)
		: FString::Printf(TEXT("玩家体力 %.0f / %.0f"), TotalStamina, TotalCapacity));
