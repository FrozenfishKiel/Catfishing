...
/** 库存格子选中事件；参数是主界面分配的格子下标，不携带鱼命令载荷。 */
DECLARE_MULTICAST_DELEGATE_OneParam(FCatInventorySlotSelected, int32);

/** 库存格子鼠标事件；主界面收到后决定选择、上下文或拖拽表现，不让格子直接执行领域命令。 */
...
protected:
	...
	/** 拖拽开始流程为随身库存物品或容器物体创建轻量 DragDropOperation；正式转移仍由目标格 Drop 和服务器命令裁决。 */
	virtual void NativeOnDragDetected(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent,
		UDragDropOperation*& OutOperation) override;

	/** Drop 流程只识别同类格之间的拖拽载荷，并把源目标格事实广播给主界面复核。 */
...
private:
	...
	/** 给 WBP 边框或颜色绑定的占用状态；它来自库存 Model 对随身库存格或鱼容器格的只读投影。 */
	UPROPERTY(BlueprintReadOnly, Transient, Category = "Catfishing|Inventory", meta = (AllowPrivateAccess = "true"))
