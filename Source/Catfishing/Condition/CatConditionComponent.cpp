...
// 草药预检流程：只读核对 authority、正式身体 runtime、ASC、倒地阈值与恢复量；不扣库存、不写 Attribute，也不制造预留状态。
...
// 进食流程：先按 RequestId 重放，再验证 authority/定义/ASC/Growth；首次按 Toxic 增加 Poison 并推进经验槽，随后只走统一倒地裁决并缓存终态。
...
// 统一恢复流程：按 Mode+RequestId 幂等重放，验证 authority/ASC/阈值 gate 后非负减少 Poison，再重新裁决 Downed 并缓存结果。
