...
{
	OutRadiusScale = 0.0;
	if (!FMath::IsFinite(InfluenceAreaMultiplier) || InfluenceAreaMultiplier <= 0.0)
	{
		return false;
	}
	OutRadiusScale = FMath::Sqrt(InfluenceAreaMultiplier);
	return FMath::IsFinite(OutRadiusScale) && OutRadiusScale > 0.0;
}
...
bool UCatChumFieldSettings::IsRuntimeReady() const
{
	...
	return bEnableChumFieldRuntime && MaxActiveFieldsPerRegion > 0 // 总开关开启，且每个水域允许的活跃窝料场数量合法
		&& TryGetInfluenceRadiusScale(InfluenceRadiusScale) // 全局面积倍率必须能安全换算成半径倍率
