...
/** 成功结算资格条件的空参数；世界进度阈值和策略来自服务器 Run 设置，不在资产节点里复制。 */
USTRUCT()
...
/** ST_RunFlow 的成功结算只读条件；资产用它在夜晚供品结算后选择成功结算夜或继续翻天。 */
...
struct CATFISHING_API FCatRunSuccessSettlementEligibleCondition : public FStateTreeConditionCommonBase
{
	...
	/** 暴露无字段实例，防止不同 ST_RunFlow 资产实例把世界进度阈值配置出第二套口径。 */
	virtual const UStruct* GetInstanceDataType() const override { return FInstanceDataType::StaticStruct(); }

	/** 从 Context Owner 的 GameMode 读取当前世界进度，再按 RunSettings 判断是否允许进入成功结算夜。 */
	virtual bool TestCondition(FStateTreeExecutionContext& Context) const override;
};
