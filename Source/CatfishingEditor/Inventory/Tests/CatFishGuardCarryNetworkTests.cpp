		...
		bool BothSidesMatch(const bool bCarried, const bool bDrop)
		{
			...
			for (int32 Peer = 0; Peer < 2; ++Peer)
			{
				...
				if (!Backpack || ACatFishPickupActor::FindCarriedFish(Character)
					|| Character->GetMouthCarriedActor() != (bCarried ? Guard : nullptr)) return false;
				...
				if (bCarried)
				{
					// 实际鱼护占用同一个猫嘴且保持可见；两端必须附着角色 Mesh 的正式 Mouth Socket。
					if (Guard->GetAttachParentActor() != Character || Guard->GetRootComponent()->GetAttachParent() != Character->GetMesh()
