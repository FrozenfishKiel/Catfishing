...
// 3. 白天只等待 DayEnded，普通夜只等待供品继续事件或 WorldProgressDepleted，并按服务器世界进度条件选择成功结算夜或下一天。
...
bool UCatRunStateTreeAuthoringLibrary::CreateOrUpdateDefaultRunFlowStateTree()
{
	...
	EnterFailureNightTask.GetInstanceData().Reason = ECatRunTransitionReason::WorldProgressDepleted;
