"""在 Unreal Editor 中创建缺失的正式 Frontend WBP，并重建 Root / Loading 以落实全局加载遮罩合同。
...
def main() -> None:
    """调用 WBP 创建、Root/Loading 重建、字体修复与设置音频入口；任一正式资产创建、编译或保存失败都会向 Editor 日志抛出明确错误。"""
    ...
    unreal.log("Catfishing Frontend WBP, Chinese fonts, and settings audio assets were prepared with the global loading overlay contract.")
