...
function Invoke-UIReachStaticCheck {
    ...
    Assert-ToolFile (Join-Path $ProjectRoot "Content\UI\HUD\WBP_CatHUD.uasset") "formal HUD WBP asset"
    Assert-ToolFile (Join-Path $ProjectRoot "Content\UI\Inventory\WBP_CatInventory.uasset") "formal Inventory WBP asset"
    Assert-ToolFile (Join-Path $ProjectRoot "Content\UI\InventorySlot\WBP_CatInventorySlot.uasset") "formal InventorySlot WBP asset"
    Assert-ToolFile (Join-Path $ProjectRoot "Content\UI\Shop\WBP_CatShop.uasset") "formal Shop WBP asset"
    Assert-ToolFile (Join-Path $ProjectRoot "Content\UI\Interaction\WBP_CatInteractionPrompt.uasset") "formal Interaction WBP asset"
    Assert-ToolFile (Join-Path $ProjectRoot "Content\UI\Collection\WBP_CatCollection.uasset") "formal Collection WBP asset"
    Assert-TextPattern 'DirectoriesToAlwaysCook=\(Path="/Game/UI/HUD"\)' "Config/DefaultGame.ini" "formal HUD directory is explicitly cooked"
    Assert-TextPattern 'DirectoriesToAlwaysCook=\(Path="/Game/UI/Inventory"\)' "Config/DefaultGame.ini" "formal Inventory directory is explicitly cooked"
    Assert-TextPattern 'DirectoriesToAlwaysCook=\(Path="/Game/UI/InventorySlot"\)' "Config/DefaultGame.ini" "formal InventorySlot directory is explicitly cooked"
    Assert-TextPattern 'DirectoriesToAlwaysCook=\(Path="/Game/UI/Shop"\)' "Config/DefaultGame.ini" "formal Shop directory is explicitly cooked"
    Assert-TextPattern 'DirectoriesToAlwaysCook=\(Path="/Game/UI/Interaction"\)' "Config/DefaultGame.ini" "formal Interaction directory is explicitly cooked"
    Assert-TextPattern 'DirectoriesToAlwaysCook=\(Path="/Game/UI/Collection"\)' "Config/DefaultGame.ini" "formal Collection directory is explicitly cooked"
    Assert-NoTextPattern 'DirectoriesToAlwaysCook=\(Path="/Game/UI"\)' "Config/DefaultGame.ini" "legacy UI root must not be broadly cooked"
    ...
    Assert-TextPattern "MouthCarrySocketName=Mouth" "Config/DefaultGame.ini" "mouth carry uses the formal mouth socket without actor offset"
