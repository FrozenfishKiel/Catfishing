...
function Invoke-ModuleStaticCheck {
    ...
    Assert-TextPattern "Definition \? Definition->GetInventoryThumbnail\(\)" "Source/Catfishing/UI/Shop/CatShopModel.cpp" "shop icon falls back to the formal inventory definition thumbnail"
    ...
    Assert-TextPattern "RemoveInventoryItemQuantityFromInstance\(BaitItemInstanceId, 1, ReservedBaitItem\)" "Source/Catfishing/Equipment/CatEquipmentComponent.cpp" "fishing begin moves one selected bait instance into its session reservation"
    Assert-TextPattern "Record->ReservedBaitDefinitionId = NAME_None" "Source/Catfishing/Equipment/CatEquipmentComponent.cpp" "fishing bait commit clears the reserved bait instead of consuming inventory twice"
    Assert-TextPattern "SubmitFishSale" "Source/Catfishing/ShopEconomy/Trading/CatShopTradeController.cpp" "fish sale coordinator"
    Assert-TextPattern "ConsumeFish" "Source/Catfishing/ShopEconomy/Trading/CatShopTradeController.cpp" "fish sale consumes Items first"
    Assert-TextPattern "ValidateFishSale" "Source/Catfishing/ShopEconomy/Trading/CatShopTradeController.cpp" "fish sale wallet precheck"
    Assert-TextPattern "ApplyFishSale" "Source/Catfishing/ShopEconomy/Trading/CatShopTradeController.cpp" "fish sale wallet apply"
    Assert-TextPattern "TryPrepareFishForSaleFromContainer" "Source/Catfishing/ShopEconomy/Trading/CatShopTradeController.cpp" "fish sale asks Items to validate its source container before payment"
    Assert-TextPattern "bContainerCanSellFish" "Source/Catfishing/Items/CatItemsService.cpp" "fish sale only admits FishGuard or SharedFishTank through the Items whitelist"
    Assert-TextPattern "PolicyUndecided" "Source/Catfishing/Items/CatItemsService.cpp" "unsupported fish sale container kinds fail closed"
    Assert-TextPattern "ValidateAddItemsFromAuthority" "Source/Catfishing/ShopEconomy/Trading/CatShopTradeController.cpp" "cart prechecks camp inventory before payment"
    Assert-TextPattern "AddItemsFromAuthority" "Source/Catfishing/ShopEconomy/Trading/CatShopTradeController.cpp" "cart delivers into camp inventory"
    Assert-TextPattern "ResolvePublicInventoryForShopOrder" "Source/Catfishing/ShopEconomy/Trading/CatShopTradeController.cpp" "server resolves camp public inventory for shop orders"
    ...
    Assert-TextPattern "ServerPublishEquipmentUnlocks" "Source/Catfishing/Framework/Game/CatfishingPlayerController.h" "profile unlock publish RPC"
    Assert-TextPattern "DOREPLIFETIME\(ThisClass, AuthorizedEquipmentUnlockIds\)" "Source/Catfishing/Framework/Game/CatfishingPlayerState.cpp" "authorized unlock replication"
