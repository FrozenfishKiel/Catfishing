...
"""把飞书知识库导出成本地镜像：docx → Markdown（带 front matter），sheet → CSV，docx 内嵌的 sheet → 旁路 CSV，bitable 跳过。
    ...
    python Scripts/export_feishu.py <space_id> <输出目录>

2026-09-15 从设计项目（已废弃）搬回本仓库 Scripts/。输出目录建议放仓库外（临时目录），
内容表定稿后按 Knowledge/Design/README.md 数据流二把对应 CSV 复制进 Knowledge/Design/。
